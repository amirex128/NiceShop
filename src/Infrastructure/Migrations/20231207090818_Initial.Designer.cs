// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using NiceShop.Infrastructure.Data;

#nullable disable

namespace NiceShop.Infrastructure.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20231207090818_Initial")]
    partial class Initial
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("ArticleCategory", b =>
                {
                    b.Property<int>("ArticlesId")
                        .HasColumnType("int");

                    b.Property<int>("CategoriesId")
                        .HasColumnType("int");

                    b.HasKey("ArticlesId", "CategoriesId");

                    b.HasIndex("CategoriesId");

                    b.ToTable("ArticleCategory");
                });

            modelBuilder.Entity("CategoryProduct", b =>
                {
                    b.Property<int>("CategoriesId")
                        .HasColumnType("int");

                    b.Property<int>("ProductsId")
                        .HasColumnType("int");

                    b.HasKey("CategoriesId", "ProductsId");

                    b.HasIndex("ProductsId");

                    b.ToTable("CategoryProduct");
                });

            modelBuilder.Entity("CouponProduct", b =>
                {
                    b.Property<int>("CouponsId")
                        .HasColumnType("int");

                    b.Property<int>("ProductsId")
                        .HasColumnType("int");

                    b.HasKey("CouponsId", "ProductsId");

                    b.HasIndex("ProductsId");

                    b.ToTable("CouponProduct");
                });

            modelBuilder.Entity("MediaProduct", b =>
                {
                    b.Property<int>("MediasId")
                        .HasColumnType("int");

                    b.Property<int>("ProductId")
                        .HasColumnType("int");

                    b.HasKey("MediasId", "ProductId");

                    b.HasIndex("ProductId");

                    b.ToTable("MediaProduct");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("Roles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("RoleClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("UserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("UserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("RoleId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("UserRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("UserTokens", (string)null);
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Address", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("AddressLine")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("CityId")
                        .HasColumnType("int");

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PostalCode")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ProvinceId")
                        .HasColumnType("int");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("CityId");

                    b.HasIndex("ProvinceId");

                    b.HasIndex("UserId");

                    b.ToTable("Addresses");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Article", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Body")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("MediaId")
                        .HasColumnType("int");

                    b.Property<string>("Slug")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Sort")
                        .HasColumnType("int");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("MediaId");

                    b.HasIndex("UserId");

                    b.ToTable("Articles");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Basket", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("CouponId")
                        .HasColumnType("int");

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("FinalPrice")
                        .HasColumnType("bigint");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("TotalDiscount")
                        .HasColumnType("bigint");

                    b.Property<long>("TotalPrice")
                        .HasColumnType("bigint");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("CouponId");

                    b.HasIndex("UserId");

                    b.ToTable("Baskets");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.BasketItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("BasketId")
                        .HasColumnType("int");

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("Price")
                        .HasColumnType("bigint");

                    b.Property<int>("ProductId")
                        .HasColumnType("int");

                    b.Property<int?>("ProductVariantId")
                        .HasColumnType("int");

                    b.Property<int>("Quantity")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("BasketId");

                    b.HasIndex("ProductId");

                    b.HasIndex("ProductVariantId");

                    b.ToTable("BasketItems");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Category", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("ParentCategoryId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ParentCategoryId");

                    b.ToTable("Categories");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.City", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ProvinceId")
                        .HasColumnType("int");

                    b.Property<string>("Slug")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("ProvinceId");

                    b.ToTable("Cities");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "اسکو",
                            ProvinceId = 1,
                            Slug = "اسکو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 2,
                            Name = "اهر",
                            ProvinceId = 1,
                            Slug = "اهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 3,
                            Name = "ایلخچی",
                            ProvinceId = 1,
                            Slug = "ایلخچی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 4,
                            Name = "آبش احمد",
                            ProvinceId = 1,
                            Slug = "آبش-احمد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 5,
                            Name = "آذرشهر",
                            ProvinceId = 1,
                            Slug = "آذرشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 6,
                            Name = "آقکند",
                            ProvinceId = 1,
                            Slug = "آقکند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 7,
                            Name = "باسمنج",
                            ProvinceId = 1,
                            Slug = "باسمنج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 8,
                            Name = "بخشایش",
                            ProvinceId = 1,
                            Slug = "بخشایش",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 9,
                            Name = "بستان آباد",
                            ProvinceId = 1,
                            Slug = "بستان-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 10,
                            Name = "بناب",
                            ProvinceId = 1,
                            Slug = "بناب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 11,
                            Name = "بناب جدید",
                            ProvinceId = 1,
                            Slug = "بناب-جدید",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 12,
                            Name = "تبریز",
                            ProvinceId = 1,
                            Slug = "تبریز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 13,
                            Name = "ترک",
                            ProvinceId = 1,
                            Slug = "ترک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 14,
                            Name = "ترکمانچای",
                            ProvinceId = 1,
                            Slug = "ترکمانچای",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 15,
                            Name = "تسوج",
                            ProvinceId = 1,
                            Slug = "تسوج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 16,
                            Name = "تیکمه داش",
                            ProvinceId = 1,
                            Slug = "تیکمه-داش",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 17,
                            Name = "جلفا",
                            ProvinceId = 1,
                            Slug = "جلفا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 18,
                            Name = "خاروانا",
                            ProvinceId = 1,
                            Slug = "خاروانا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 19,
                            Name = "خامنه",
                            ProvinceId = 1,
                            Slug = "خامنه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 20,
                            Name = "خراجو",
                            ProvinceId = 1,
                            Slug = "خراجو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 21,
                            Name = "خسروشهر",
                            ProvinceId = 1,
                            Slug = "خسروشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 22,
                            Name = "خضرلو",
                            ProvinceId = 1,
                            Slug = "خضرلو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 23,
                            Name = "خمارلو",
                            ProvinceId = 1,
                            Slug = "خمارلو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 24,
                            Name = "خواجه",
                            ProvinceId = 1,
                            Slug = "خواجه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 25,
                            Name = "دوزدوزان",
                            ProvinceId = 1,
                            Slug = "دوزدوزان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 26,
                            Name = "زرنق",
                            ProvinceId = 1,
                            Slug = "زرنق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 27,
                            Name = "زنوز",
                            ProvinceId = 1,
                            Slug = "زنوز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 28,
                            Name = "سراب",
                            ProvinceId = 1,
                            Slug = "سراب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 29,
                            Name = "سردرود",
                            ProvinceId = 1,
                            Slug = "سردرود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 30,
                            Name = "سهند",
                            ProvinceId = 1,
                            Slug = "سهند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 31,
                            Name = "سیس",
                            ProvinceId = 1,
                            Slug = "سیس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 32,
                            Name = "سیه رود",
                            ProvinceId = 1,
                            Slug = "سیه-رود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 33,
                            Name = "شبستر",
                            ProvinceId = 1,
                            Slug = "شبستر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 34,
                            Name = "شربیان",
                            ProvinceId = 1,
                            Slug = "شربیان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 35,
                            Name = "شرفخانه",
                            ProvinceId = 1,
                            Slug = "شرفخانه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 36,
                            Name = "شندآباد",
                            ProvinceId = 1,
                            Slug = "شندآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 37,
                            Name = "صوفیان",
                            ProvinceId = 1,
                            Slug = "صوفیان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 38,
                            Name = "عجب شیر",
                            ProvinceId = 1,
                            Slug = "عجب-شیر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 39,
                            Name = "قره آغاج",
                            ProvinceId = 1,
                            Slug = "قره-آغاج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 40,
                            Name = "کشکسرای",
                            ProvinceId = 1,
                            Slug = "کشکسرای",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 41,
                            Name = "کلوانق",
                            ProvinceId = 1,
                            Slug = "کلوانق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 42,
                            Name = "کلیبر",
                            ProvinceId = 1,
                            Slug = "کلیبر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 43,
                            Name = "کوزه کنان",
                            ProvinceId = 1,
                            Slug = "کوزه-کنان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 44,
                            Name = "گوگان",
                            ProvinceId = 1,
                            Slug = "گوگان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 45,
                            Name = "لیلان",
                            ProvinceId = 1,
                            Slug = "لیلان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 46,
                            Name = "مراغه",
                            ProvinceId = 1,
                            Slug = "مراغه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 47,
                            Name = "مرند",
                            ProvinceId = 1,
                            Slug = "مرند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 48,
                            Name = "ملکان",
                            ProvinceId = 1,
                            Slug = "ملکان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 49,
                            Name = "ملک کیان",
                            ProvinceId = 1,
                            Slug = "ملک-کیان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 50,
                            Name = "ممقان",
                            ProvinceId = 1,
                            Slug = "ممقان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 51,
                            Name = "مهربان",
                            ProvinceId = 1,
                            Slug = "مهربان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 52,
                            Name = "میانه",
                            ProvinceId = 1,
                            Slug = "میانه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 53,
                            Name = "نظرکهریزی",
                            ProvinceId = 1,
                            Slug = "نظرکهریزی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 54,
                            Name = "هادی شهر",
                            ProvinceId = 1,
                            Slug = "هادی-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 55,
                            Name = "هرگلان",
                            ProvinceId = 1,
                            Slug = "هرگلان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 56,
                            Name = "هریس",
                            ProvinceId = 1,
                            Slug = "هریس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 57,
                            Name = "هشترود",
                            ProvinceId = 1,
                            Slug = "هشترود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 58,
                            Name = "هوراند",
                            ProvinceId = 1,
                            Slug = "هوراند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 59,
                            Name = "وایقان",
                            ProvinceId = 1,
                            Slug = "وایقان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 60,
                            Name = "ورزقان",
                            ProvinceId = 1,
                            Slug = "ورزقان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 61,
                            Name = "یامچی",
                            ProvinceId = 1,
                            Slug = "یامچی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 62,
                            Name = "ارومیه",
                            ProvinceId = 2,
                            Slug = "ارومیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 63,
                            Name = "اشنویه",
                            ProvinceId = 2,
                            Slug = "اشنویه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 64,
                            Name = "ایواوغلی",
                            ProvinceId = 2,
                            Slug = "ایواوغلی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 65,
                            Name = "آواجیق",
                            ProvinceId = 2,
                            Slug = "آواجیق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 66,
                            Name = "باروق",
                            ProvinceId = 2,
                            Slug = "باروق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 67,
                            Name = "بازرگان",
                            ProvinceId = 2,
                            Slug = "بازرگان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 68,
                            Name = "بوکان",
                            ProvinceId = 2,
                            Slug = "بوکان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 69,
                            Name = "پلدشت",
                            ProvinceId = 2,
                            Slug = "پلدشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 70,
                            Name = "پیرانشهر",
                            ProvinceId = 2,
                            Slug = "پیرانشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 71,
                            Name = "تازه شهر",
                            ProvinceId = 2,
                            Slug = "تازه-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 72,
                            Name = "تکاب",
                            ProvinceId = 2,
                            Slug = "تکاب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 73,
                            Name = "چهاربرج",
                            ProvinceId = 2,
                            Slug = "چهاربرج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 74,
                            Name = "خوی",
                            ProvinceId = 2,
                            Slug = "خوی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 75,
                            Name = "دیزج دیز",
                            ProvinceId = 2,
                            Slug = "دیزج-دیز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 76,
                            Name = "ربط",
                            ProvinceId = 2,
                            Slug = "ربط",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 77,
                            Name = "سردشت",
                            ProvinceId = 2,
                            Slug = "آذربایجان-غربی-سردشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 78,
                            Name = "سرو",
                            ProvinceId = 2,
                            Slug = "سرو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 79,
                            Name = "سلماس",
                            ProvinceId = 2,
                            Slug = "سلماس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 80,
                            Name = "سیلوانه",
                            ProvinceId = 2,
                            Slug = "سیلوانه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 81,
                            Name = "سیمینه",
                            ProvinceId = 2,
                            Slug = "سیمینه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 82,
                            Name = "سیه چشمه",
                            ProvinceId = 2,
                            Slug = "سیه-چشمه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 83,
                            Name = "شاهین دژ",
                            ProvinceId = 2,
                            Slug = "شاهین-دژ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 84,
                            Name = "شوط",
                            ProvinceId = 2,
                            Slug = "شوط",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 85,
                            Name = "فیرورق",
                            ProvinceId = 2,
                            Slug = "فیرورق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 86,
                            Name = "قره ضیاءالدین",
                            ProvinceId = 2,
                            Slug = "قره-ضیاءالدین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 87,
                            Name = "قطور",
                            ProvinceId = 2,
                            Slug = "قطور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 88,
                            Name = "قوشچی",
                            ProvinceId = 2,
                            Slug = "قوشچی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 89,
                            Name = "کشاورز",
                            ProvinceId = 2,
                            Slug = "کشاورز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 90,
                            Name = "گردکشانه",
                            ProvinceId = 2,
                            Slug = "گردکشانه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 91,
                            Name = "ماکو",
                            ProvinceId = 2,
                            Slug = "ماکو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 92,
                            Name = "محمدیار",
                            ProvinceId = 2,
                            Slug = "محمدیار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 93,
                            Name = "محمودآباد",
                            ProvinceId = 2,
                            Slug = "آذربایجان-غربی-محمودآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 94,
                            Name = "مهاباد",
                            ProvinceId = 2,
                            Slug = "آذربایجان-غربی-مهاباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 95,
                            Name = "میاندوآب",
                            ProvinceId = 2,
                            Slug = "میاندوآب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 96,
                            Name = "میرآباد",
                            ProvinceId = 2,
                            Slug = "میرآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 97,
                            Name = "نالوس",
                            ProvinceId = 2,
                            Slug = "نالوس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 98,
                            Name = "نقده",
                            ProvinceId = 2,
                            Slug = "نقده",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 99,
                            Name = "نوشین",
                            ProvinceId = 2,
                            Slug = "نوشین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 100,
                            Name = "اردبیل",
                            ProvinceId = 3,
                            Slug = "اردبیل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 101,
                            Name = "اصلاندوز",
                            ProvinceId = 3,
                            Slug = "اصلاندوز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 102,
                            Name = "آبی بیگلو",
                            ProvinceId = 3,
                            Slug = "آبی-بیگلو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 103,
                            Name = "بیله سوار",
                            ProvinceId = 3,
                            Slug = "بیله-سوار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 104,
                            Name = "پارس آباد",
                            ProvinceId = 3,
                            Slug = "پارس-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 105,
                            Name = "تازه کند",
                            ProvinceId = 3,
                            Slug = "تازه-کند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 106,
                            Name = "تازه کندانگوت",
                            ProvinceId = 3,
                            Slug = "تازه-کندانگوت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 107,
                            Name = "جعفرآباد",
                            ProvinceId = 3,
                            Slug = "جعفرآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 108,
                            Name = "خلخال",
                            ProvinceId = 3,
                            Slug = "خلخال",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 109,
                            Name = "رضی",
                            ProvinceId = 3,
                            Slug = "رضی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 110,
                            Name = "سرعین",
                            ProvinceId = 3,
                            Slug = "سرعین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 111,
                            Name = "عنبران",
                            ProvinceId = 3,
                            Slug = "عنبران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 112,
                            Name = "فخرآباد",
                            ProvinceId = 3,
                            Slug = "فخرآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 113,
                            Name = "کلور",
                            ProvinceId = 3,
                            Slug = "کلور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 114,
                            Name = "کوراییم",
                            ProvinceId = 3,
                            Slug = "کوراییم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 115,
                            Name = "گرمی",
                            ProvinceId = 3,
                            Slug = "گرمی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 116,
                            Name = "گیوی",
                            ProvinceId = 3,
                            Slug = "گیوی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 117,
                            Name = "لاهرود",
                            ProvinceId = 3,
                            Slug = "لاهرود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 118,
                            Name = "مشگین شهر",
                            ProvinceId = 3,
                            Slug = "مشگین-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 119,
                            Name = "نمین",
                            ProvinceId = 3,
                            Slug = "نمین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 120,
                            Name = "نیر",
                            ProvinceId = 3,
                            Slug = "اردبیل-نیر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 121,
                            Name = "هشتجین",
                            ProvinceId = 3,
                            Slug = "هشتجین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 122,
                            Name = "هیر",
                            ProvinceId = 3,
                            Slug = "هیر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 123,
                            Name = "ابریشم",
                            ProvinceId = 4,
                            Slug = "ابریشم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 124,
                            Name = "ابوزیدآباد",
                            ProvinceId = 4,
                            Slug = "ابوزیدآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 125,
                            Name = "اردستان",
                            ProvinceId = 4,
                            Slug = "اردستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 126,
                            Name = "اژیه",
                            ProvinceId = 4,
                            Slug = "اژیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 127,
                            Name = "اصفهان",
                            ProvinceId = 4,
                            Slug = "اصفهان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 128,
                            Name = "افوس",
                            ProvinceId = 4,
                            Slug = "افوس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 129,
                            Name = "انارک",
                            ProvinceId = 4,
                            Slug = "انارک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 130,
                            Name = "ایمانشهر",
                            ProvinceId = 4,
                            Slug = "ایمانشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 131,
                            Name = "آران وبیدگل",
                            ProvinceId = 4,
                            Slug = "آران-وبیدگل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 132,
                            Name = "بادرود",
                            ProvinceId = 4,
                            Slug = "بادرود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 133,
                            Name = "باغ بهادران",
                            ProvinceId = 4,
                            Slug = "باغ-بهادران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 134,
                            Name = "بافران",
                            ProvinceId = 4,
                            Slug = "بافران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 135,
                            Name = "برزک",
                            ProvinceId = 4,
                            Slug = "برزک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 136,
                            Name = "برف انبار",
                            ProvinceId = 4,
                            Slug = "برف-انبار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 137,
                            Name = "بهاران شهر",
                            ProvinceId = 4,
                            Slug = "بهاران-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 138,
                            Name = "بهارستان",
                            ProvinceId = 4,
                            Slug = "بهارستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 139,
                            Name = "بوئین و میاندشت",
                            ProvinceId = 4,
                            Slug = "بوئین-میاندشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 140,
                            Name = "پیربکران",
                            ProvinceId = 4,
                            Slug = "پیربکران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 141,
                            Name = "تودشک",
                            ProvinceId = 4,
                            Slug = "تودشک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 142,
                            Name = "تیران",
                            ProvinceId = 4,
                            Slug = "تیران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 143,
                            Name = "جندق",
                            ProvinceId = 4,
                            Slug = "جندق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 144,
                            Name = "جوزدان",
                            ProvinceId = 4,
                            Slug = "جوزدان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 145,
                            Name = "جوشقان و کامو",
                            ProvinceId = 4,
                            Slug = "جوشقان-کامو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 146,
                            Name = "چادگان",
                            ProvinceId = 4,
                            Slug = "چادگان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 147,
                            Name = "چرمهین",
                            ProvinceId = 4,
                            Slug = "چرمهین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 148,
                            Name = "چمگردان",
                            ProvinceId = 4,
                            Slug = "چمگردان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 149,
                            Name = "حبیب آباد",
                            ProvinceId = 4,
                            Slug = "حبیب-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 150,
                            Name = "حسن آباد",
                            ProvinceId = 4,
                            Slug = "اصفهان-حسن-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 151,
                            Name = "حنا",
                            ProvinceId = 4,
                            Slug = "حنا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 152,
                            Name = "خالدآباد",
                            ProvinceId = 4,
                            Slug = "خالدآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 153,
                            Name = "خمینی شهر",
                            ProvinceId = 4,
                            Slug = "خمینی-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 154,
                            Name = "خوانسار",
                            ProvinceId = 4,
                            Slug = "خوانسار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 155,
                            Name = "خور",
                            ProvinceId = 4,
                            Slug = "اصفهان-خور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 157,
                            Name = "خورزوق",
                            ProvinceId = 4,
                            Slug = "خورزوق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 158,
                            Name = "داران",
                            ProvinceId = 4,
                            Slug = "داران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 159,
                            Name = "دامنه",
                            ProvinceId = 4,
                            Slug = "دامنه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 160,
                            Name = "درچه",
                            ProvinceId = 4,
                            Slug = "درچه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 161,
                            Name = "دستگرد",
                            ProvinceId = 4,
                            Slug = "دستگرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 162,
                            Name = "دهاقان",
                            ProvinceId = 4,
                            Slug = "دهاقان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 163,
                            Name = "دهق",
                            ProvinceId = 4,
                            Slug = "دهق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 164,
                            Name = "دولت آباد",
                            ProvinceId = 4,
                            Slug = "اصفهان-دولت-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 165,
                            Name = "دیزیچه",
                            ProvinceId = 4,
                            Slug = "دیزیچه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 166,
                            Name = "رزوه",
                            ProvinceId = 4,
                            Slug = "رزوه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 167,
                            Name = "رضوانشهر",
                            ProvinceId = 4,
                            Slug = "اصفهان-رضوانشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 168,
                            Name = "زاینده رود",
                            ProvinceId = 4,
                            Slug = "زاینده-رود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 169,
                            Name = "زرین شهر",
                            ProvinceId = 4,
                            Slug = "زرین-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 170,
                            Name = "زواره",
                            ProvinceId = 4,
                            Slug = "زواره",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 171,
                            Name = "زیباشهر",
                            ProvinceId = 4,
                            Slug = "زیباشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 172,
                            Name = "سده لنجان",
                            ProvinceId = 4,
                            Slug = "سده-لنجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 173,
                            Name = "سفیدشهر",
                            ProvinceId = 4,
                            Slug = "سفیدشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 174,
                            Name = "سگزی",
                            ProvinceId = 4,
                            Slug = "سگزی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 175,
                            Name = "سمیرم",
                            ProvinceId = 4,
                            Slug = "سمیرم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 176,
                            Name = "شاهین شهر",
                            ProvinceId = 4,
                            Slug = "شاهین-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 177,
                            Name = "شهرضا",
                            ProvinceId = 4,
                            Slug = "شهرضا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 178,
                            Name = "طالخونچه",
                            ProvinceId = 4,
                            Slug = "طالخونچه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 179,
                            Name = "عسگران",
                            ProvinceId = 4,
                            Slug = "عسگران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 180,
                            Name = "علویجه",
                            ProvinceId = 4,
                            Slug = "علویجه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 181,
                            Name = "فرخی",
                            ProvinceId = 4,
                            Slug = "فرخی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 182,
                            Name = "فریدونشهر",
                            ProvinceId = 4,
                            Slug = "فریدونشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 183,
                            Name = "فلاورجان",
                            ProvinceId = 4,
                            Slug = "فلاورجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 184,
                            Name = "فولادشهر",
                            ProvinceId = 4,
                            Slug = "فولادشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 185,
                            Name = "قمصر",
                            ProvinceId = 4,
                            Slug = "قمصر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 186,
                            Name = "قهجاورستان",
                            ProvinceId = 4,
                            Slug = "قهجاورستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 187,
                            Name = "قهدریجان",
                            ProvinceId = 4,
                            Slug = "قهدریجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 188,
                            Name = "کاشان",
                            ProvinceId = 4,
                            Slug = "کاشان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 189,
                            Name = "کرکوند",
                            ProvinceId = 4,
                            Slug = "کرکوند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 190,
                            Name = "کلیشاد و سودرجان",
                            ProvinceId = 4,
                            Slug = "کلیشاد-سودرجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 191,
                            Name = "کمشچه",
                            ProvinceId = 4,
                            Slug = "کمشچه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 192,
                            Name = "کمه",
                            ProvinceId = 4,
                            Slug = "کمه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 193,
                            Name = "کهریزسنگ",
                            ProvinceId = 4,
                            Slug = "کهریزسنگ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 194,
                            Name = "کوشک",
                            ProvinceId = 4,
                            Slug = "کوشک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 195,
                            Name = "کوهپایه",
                            ProvinceId = 4,
                            Slug = "کوهپایه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 196,
                            Name = "گرگاب",
                            ProvinceId = 4,
                            Slug = "گرگاب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 197,
                            Name = "گزبرخوار",
                            ProvinceId = 4,
                            Slug = "گزبرخوار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 198,
                            Name = "گلپایگان",
                            ProvinceId = 4,
                            Slug = "گلپایگان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 199,
                            Name = "گلدشت",
                            ProvinceId = 4,
                            Slug = "گلدشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 200,
                            Name = "گلشهر",
                            ProvinceId = 4,
                            Slug = "گلشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 201,
                            Name = "گوگد",
                            ProvinceId = 4,
                            Slug = "گوگد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 202,
                            Name = "لای بید",
                            ProvinceId = 4,
                            Slug = "لای-بید",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 203,
                            Name = "مبارکه",
                            ProvinceId = 4,
                            Slug = "مبارکه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 204,
                            Name = "مجلسی",
                            ProvinceId = 4,
                            Slug = "مجلسی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 205,
                            Name = "محمدآباد",
                            ProvinceId = 4,
                            Slug = "اصفهان-محمدآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 206,
                            Name = "مشکات",
                            ProvinceId = 4,
                            Slug = "مشکات",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 207,
                            Name = "منظریه",
                            ProvinceId = 4,
                            Slug = "منظریه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 208,
                            Name = "مهاباد",
                            ProvinceId = 4,
                            Slug = "اصفهان-مهاباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 209,
                            Name = "میمه",
                            ProvinceId = 4,
                            Slug = "اصفهان-میمه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 210,
                            Name = "نائین",
                            ProvinceId = 4,
                            Slug = "نائین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 211,
                            Name = "نجف آباد",
                            ProvinceId = 4,
                            Slug = "نجف-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 212,
                            Name = "نصرآباد",
                            ProvinceId = 4,
                            Slug = "اصفهان-نصرآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 213,
                            Name = "نطنز",
                            ProvinceId = 4,
                            Slug = "نطنز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 214,
                            Name = "نوش آباد",
                            ProvinceId = 4,
                            Slug = "نوش-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 215,
                            Name = "نیاسر",
                            ProvinceId = 4,
                            Slug = "نیاسر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 216,
                            Name = "نیک آباد",
                            ProvinceId = 4,
                            Slug = "نیک-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 217,
                            Name = "هرند",
                            ProvinceId = 4,
                            Slug = "هرند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 218,
                            Name = "ورزنه",
                            ProvinceId = 4,
                            Slug = "ورزنه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 219,
                            Name = "ورنامخواست",
                            ProvinceId = 4,
                            Slug = "ورنامخواست",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 220,
                            Name = "وزوان",
                            ProvinceId = 4,
                            Slug = "وزوان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 221,
                            Name = "ونک",
                            ProvinceId = 4,
                            Slug = "ونک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 222,
                            Name = "اسارا",
                            ProvinceId = 5,
                            Slug = "اسارا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 223,
                            Name = "اشتهارد",
                            ProvinceId = 5,
                            Slug = "اشتهارد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 224,
                            Name = "تنکمان",
                            ProvinceId = 5,
                            Slug = "تنکمان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 225,
                            Name = "چهارباغ",
                            ProvinceId = 5,
                            Slug = "چهارباغ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 226,
                            Name = "سعید آباد",
                            ProvinceId = 5,
                            Slug = "سعید-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 227,
                            Name = "شهر جدید هشتگرد",
                            ProvinceId = 5,
                            Slug = "شهر-جدید-هشتگرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 228,
                            Name = "طالقان",
                            ProvinceId = 5,
                            Slug = "طالقان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 229,
                            Name = "کرج",
                            ProvinceId = 5,
                            Slug = "کرج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 230,
                            Name = "کمال شهر",
                            ProvinceId = 5,
                            Slug = "کمال-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 231,
                            Name = "کوهسار",
                            ProvinceId = 5,
                            Slug = "کوهسار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 232,
                            Name = "گرمدره",
                            ProvinceId = 5,
                            Slug = "گرمدره",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 233,
                            Name = "ماهدشت",
                            ProvinceId = 5,
                            Slug = "ماهدشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 234,
                            Name = "محمدشهر",
                            ProvinceId = 5,
                            Slug = "البرز-محمدشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 235,
                            Name = "مشکین دشت",
                            ProvinceId = 5,
                            Slug = "مشکین-دشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 236,
                            Name = "نظرآباد",
                            ProvinceId = 5,
                            Slug = "نظرآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 237,
                            Name = "هشتگرد",
                            ProvinceId = 5,
                            Slug = "هشتگرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 238,
                            Name = "ارکواز",
                            ProvinceId = 6,
                            Slug = "ارکواز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 239,
                            Name = "ایلام",
                            ProvinceId = 6,
                            Slug = "ایلام",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 240,
                            Name = "ایوان",
                            ProvinceId = 6,
                            Slug = "ایوان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 241,
                            Name = "آبدانان",
                            ProvinceId = 6,
                            Slug = "آبدانان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 242,
                            Name = "آسمان آباد",
                            ProvinceId = 6,
                            Slug = "آسمان-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 243,
                            Name = "بدره",
                            ProvinceId = 6,
                            Slug = "بدره",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 244,
                            Name = "پهله",
                            ProvinceId = 6,
                            Slug = "پهله",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 245,
                            Name = "توحید",
                            ProvinceId = 6,
                            Slug = "توحید",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 246,
                            Name = "چوار",
                            ProvinceId = 6,
                            Slug = "چوار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 247,
                            Name = "دره شهر",
                            ProvinceId = 6,
                            Slug = "دره-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 248,
                            Name = "دلگشا",
                            ProvinceId = 6,
                            Slug = "دلگشا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 249,
                            Name = "دهلران",
                            ProvinceId = 6,
                            Slug = "دهلران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 250,
                            Name = "زرنه",
                            ProvinceId = 6,
                            Slug = "زرنه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 251,
                            Name = "سراب باغ",
                            ProvinceId = 6,
                            Slug = "سراب-باغ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 252,
                            Name = "سرابله",
                            ProvinceId = 6,
                            Slug = "سرابله",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 253,
                            Name = "صالح آباد",
                            ProvinceId = 6,
                            Slug = "ایلام-صالح-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 254,
                            Name = "لومار",
                            ProvinceId = 6,
                            Slug = "لومار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 255,
                            Name = "مهران",
                            ProvinceId = 6,
                            Slug = "مهران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 256,
                            Name = "مورموری",
                            ProvinceId = 6,
                            Slug = "مورموری",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 257,
                            Name = "موسیان",
                            ProvinceId = 6,
                            Slug = "موسیان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 258,
                            Name = "میمه",
                            ProvinceId = 6,
                            Slug = "ایلام-میمه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 259,
                            Name = "امام حسن",
                            ProvinceId = 7,
                            Slug = "امام-حسن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 260,
                            Name = "انارستان",
                            ProvinceId = 7,
                            Slug = "انارستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 261,
                            Name = "اهرم",
                            ProvinceId = 7,
                            Slug = "اهرم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 262,
                            Name = "آب پخش",
                            ProvinceId = 7,
                            Slug = "آب-پخش",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 263,
                            Name = "آبدان",
                            ProvinceId = 7,
                            Slug = "آبدان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 264,
                            Name = "برازجان",
                            ProvinceId = 7,
                            Slug = "برازجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 265,
                            Name = "بردخون",
                            ProvinceId = 7,
                            Slug = "بردخون",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 266,
                            Name = "بندردیر",
                            ProvinceId = 7,
                            Slug = "بندردیر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 267,
                            Name = "بندردیلم",
                            ProvinceId = 7,
                            Slug = "بندردیلم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 268,
                            Name = "بندرریگ",
                            ProvinceId = 7,
                            Slug = "بندرریگ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 269,
                            Name = "بندرکنگان",
                            ProvinceId = 7,
                            Slug = "بندرکنگان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 270,
                            Name = "بندرگناوه",
                            ProvinceId = 7,
                            Slug = "بندرگناوه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 271,
                            Name = "بنک",
                            ProvinceId = 7,
                            Slug = "بنک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 272,
                            Name = "بوشهر",
                            ProvinceId = 7,
                            Slug = "بوشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 273,
                            Name = "تنگ ارم",
                            ProvinceId = 7,
                            Slug = "تنگ-ارم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 274,
                            Name = "جم",
                            ProvinceId = 7,
                            Slug = "جم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 275,
                            Name = "چغادک",
                            ProvinceId = 7,
                            Slug = "چغادک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 276,
                            Name = "خارک",
                            ProvinceId = 7,
                            Slug = "خارک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 277,
                            Name = "خورموج",
                            ProvinceId = 7,
                            Slug = "خورموج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 278,
                            Name = "دالکی",
                            ProvinceId = 7,
                            Slug = "دالکی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 279,
                            Name = "دلوار",
                            ProvinceId = 7,
                            Slug = "دلوار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 280,
                            Name = "ریز",
                            ProvinceId = 7,
                            Slug = "ریز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 281,
                            Name = "سعدآباد",
                            ProvinceId = 7,
                            Slug = "سعدآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 282,
                            Name = "سیراف",
                            ProvinceId = 7,
                            Slug = "سیراف",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 283,
                            Name = "شبانکاره",
                            ProvinceId = 7,
                            Slug = "شبانکاره",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 284,
                            Name = "شنبه",
                            ProvinceId = 7,
                            Slug = "شنبه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 285,
                            Name = "عسلویه",
                            ProvinceId = 7,
                            Slug = "عسلویه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 286,
                            Name = "کاکی",
                            ProvinceId = 7,
                            Slug = "کاکی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 287,
                            Name = "کلمه",
                            ProvinceId = 7,
                            Slug = "کلمه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 288,
                            Name = "نخل تقی",
                            ProvinceId = 7,
                            Slug = "نخل-تقی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 289,
                            Name = "وحدتیه",
                            ProvinceId = 7,
                            Slug = "وحدتیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 290,
                            Name = "ارجمند",
                            ProvinceId = 8,
                            Slug = "ارجمند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 291,
                            Name = "اسلامشهر",
                            ProvinceId = 8,
                            Slug = "اسلامشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 292,
                            Name = "اندیشه",
                            ProvinceId = 8,
                            Slug = "اندیشه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 293,
                            Name = "آبسرد",
                            ProvinceId = 8,
                            Slug = "آبسرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 294,
                            Name = "آبعلی",
                            ProvinceId = 8,
                            Slug = "آبعلی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 295,
                            Name = "باغستان",
                            ProvinceId = 8,
                            Slug = "باغستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 296,
                            Name = "باقرشهر",
                            ProvinceId = 8,
                            Slug = "باقرشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 297,
                            Name = "بومهن",
                            ProvinceId = 8,
                            Slug = "بومهن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 298,
                            Name = "پاکدشت",
                            ProvinceId = 8,
                            Slug = "پاکدشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 299,
                            Name = "پردیس",
                            ProvinceId = 8,
                            Slug = "پردیس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 300,
                            Name = "پیشوا",
                            ProvinceId = 8,
                            Slug = "پیشوا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 301,
                            Name = "تهران",
                            ProvinceId = 8,
                            Slug = "تهران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 302,
                            Name = "جوادآباد",
                            ProvinceId = 8,
                            Slug = "جوادآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 303,
                            Name = "چهاردانگه",
                            ProvinceId = 8,
                            Slug = "چهاردانگه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 304,
                            Name = "حسن آباد",
                            ProvinceId = 8,
                            Slug = "تهران-حسن-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 305,
                            Name = "دماوند",
                            ProvinceId = 8,
                            Slug = "دماوند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 306,
                            Name = "دیزین",
                            ProvinceId = 8,
                            Slug = "دیزین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 307,
                            Name = "شهر ری",
                            ProvinceId = 8,
                            Slug = "شهر-ری",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 308,
                            Name = "رباط کریم",
                            ProvinceId = 8,
                            Slug = "رباط-کریم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 309,
                            Name = "رودهن",
                            ProvinceId = 8,
                            Slug = "رودهن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 310,
                            Name = "شاهدشهر",
                            ProvinceId = 8,
                            Slug = "شاهدشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 311,
                            Name = "شریف آباد",
                            ProvinceId = 8,
                            Slug = "شریف-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 312,
                            Name = "شمشک",
                            ProvinceId = 8,
                            Slug = "شمشک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 313,
                            Name = "شهریار",
                            ProvinceId = 8,
                            Slug = "شهریار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 314,
                            Name = "صالح آباد",
                            ProvinceId = 8,
                            Slug = "تهران-صالح-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 315,
                            Name = "صباشهر",
                            ProvinceId = 8,
                            Slug = "صباشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 316,
                            Name = "صفادشت",
                            ProvinceId = 8,
                            Slug = "صفادشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 317,
                            Name = "فردوسیه",
                            ProvinceId = 8,
                            Slug = "فردوسیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 318,
                            Name = "فشم",
                            ProvinceId = 8,
                            Slug = "فشم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 319,
                            Name = "فیروزکوه",
                            ProvinceId = 8,
                            Slug = "فیروزکوه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 320,
                            Name = "قدس",
                            ProvinceId = 8,
                            Slug = "قدس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 321,
                            Name = "قرچک",
                            ProvinceId = 8,
                            Slug = "قرچک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 322,
                            Name = "کهریزک",
                            ProvinceId = 8,
                            Slug = "کهریزک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 323,
                            Name = "کیلان",
                            ProvinceId = 8,
                            Slug = "کیلان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 324,
                            Name = "گلستان",
                            ProvinceId = 8,
                            Slug = "شهر-گلستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 325,
                            Name = "لواسان",
                            ProvinceId = 8,
                            Slug = "لواسان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 326,
                            Name = "ملارد",
                            ProvinceId = 8,
                            Slug = "ملارد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 327,
                            Name = "میگون",
                            ProvinceId = 8,
                            Slug = "میگون",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 328,
                            Name = "نسیم شهر",
                            ProvinceId = 8,
                            Slug = "نسیم-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 329,
                            Name = "نصیرآباد",
                            ProvinceId = 8,
                            Slug = "نصیرآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 330,
                            Name = "وحیدیه",
                            ProvinceId = 8,
                            Slug = "وحیدیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 331,
                            Name = "ورامین",
                            ProvinceId = 8,
                            Slug = "ورامین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 332,
                            Name = "اردل",
                            ProvinceId = 9,
                            Slug = "اردل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 333,
                            Name = "آلونی",
                            ProvinceId = 9,
                            Slug = "آلونی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 334,
                            Name = "باباحیدر",
                            ProvinceId = 9,
                            Slug = "باباحیدر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 335,
                            Name = "بروجن",
                            ProvinceId = 9,
                            Slug = "بروجن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 336,
                            Name = "بلداجی",
                            ProvinceId = 9,
                            Slug = "بلداجی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 337,
                            Name = "بن",
                            ProvinceId = 9,
                            Slug = "بن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 338,
                            Name = "جونقان",
                            ProvinceId = 9,
                            Slug = "جونقان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 339,
                            Name = "چلگرد",
                            ProvinceId = 9,
                            Slug = "چلگرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 340,
                            Name = "سامان",
                            ProvinceId = 9,
                            Slug = "سامان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 341,
                            Name = "سفیددشت",
                            ProvinceId = 9,
                            Slug = "سفیددشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 342,
                            Name = "سودجان",
                            ProvinceId = 9,
                            Slug = "سودجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 343,
                            Name = "سورشجان",
                            ProvinceId = 9,
                            Slug = "سورشجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 344,
                            Name = "شلمزار",
                            ProvinceId = 9,
                            Slug = "شلمزار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 345,
                            Name = "شهرکرد",
                            ProvinceId = 9,
                            Slug = "شهرکرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 346,
                            Name = "طاقانک",
                            ProvinceId = 9,
                            Slug = "طاقانک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 347,
                            Name = "فارسان",
                            ProvinceId = 9,
                            Slug = "فارسان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 348,
                            Name = "فرادنبه",
                            ProvinceId = 9,
                            Slug = "فرادبنه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 349,
                            Name = "فرخ شهر",
                            ProvinceId = 9,
                            Slug = "فرخ-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 350,
                            Name = "کیان",
                            ProvinceId = 9,
                            Slug = "کیان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 351,
                            Name = "گندمان",
                            ProvinceId = 9,
                            Slug = "گندمان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 352,
                            Name = "گهرو",
                            ProvinceId = 9,
                            Slug = "گهرو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 353,
                            Name = "لردگان",
                            ProvinceId = 9,
                            Slug = "لردگان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 354,
                            Name = "مال خلیفه",
                            ProvinceId = 9,
                            Slug = "مال-خلیفه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 355,
                            Name = "ناغان",
                            ProvinceId = 9,
                            Slug = "ناغان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 356,
                            Name = "نافچ",
                            ProvinceId = 9,
                            Slug = "نافچ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 357,
                            Name = "نقنه",
                            ProvinceId = 9,
                            Slug = "نقنه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 358,
                            Name = "هفشجان",
                            ProvinceId = 9,
                            Slug = "هفشجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 359,
                            Name = "ارسک",
                            ProvinceId = 10,
                            Slug = "ارسک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 360,
                            Name = "اسدیه",
                            ProvinceId = 10,
                            Slug = "اسدیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 361,
                            Name = "اسفدن",
                            ProvinceId = 10,
                            Slug = "اسفدن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 362,
                            Name = "اسلامیه",
                            ProvinceId = 10,
                            Slug = "اسلامیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 363,
                            Name = "آرین شهر",
                            ProvinceId = 10,
                            Slug = "آرین-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 364,
                            Name = "آیسک",
                            ProvinceId = 10,
                            Slug = "آیسک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 365,
                            Name = "بشرویه",
                            ProvinceId = 10,
                            Slug = "بشرویه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 366,
                            Name = "بیرجند",
                            ProvinceId = 10,
                            Slug = "بیرجند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 367,
                            Name = "حاجی آباد",
                            ProvinceId = 10,
                            Slug = "خراسان-جنوبی-حاجی-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 368,
                            Name = "خضری دشت بیاض",
                            ProvinceId = 10,
                            Slug = "خضری-دشت-بیاض",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 369,
                            Name = "خوسف",
                            ProvinceId = 10,
                            Slug = "خوسف",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 370,
                            Name = "زهان",
                            ProvinceId = 10,
                            Slug = "زهان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 371,
                            Name = "سرایان",
                            ProvinceId = 10,
                            Slug = "سرایان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 372,
                            Name = "سربیشه",
                            ProvinceId = 10,
                            Slug = "سربیشه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 373,
                            Name = "سه قلعه",
                            ProvinceId = 10,
                            Slug = "سه-قلعه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 374,
                            Name = "شوسف",
                            ProvinceId = 10,
                            Slug = "شوسف",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 375,
                            Name = "طبس ",
                            ProvinceId = 10,
                            Slug = "خراسان-جنوبی-طبس-",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 376,
                            Name = "فردوس",
                            ProvinceId = 10,
                            Slug = "فردوس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 377,
                            Name = "قاین",
                            ProvinceId = 10,
                            Slug = "قاین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 378,
                            Name = "قهستان",
                            ProvinceId = 10,
                            Slug = "قهستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 379,
                            Name = "محمدشهر",
                            ProvinceId = 10,
                            Slug = "خراسان-جنوبی-محمدشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 380,
                            Name = "مود",
                            ProvinceId = 10,
                            Slug = "مود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 381,
                            Name = "نهبندان",
                            ProvinceId = 10,
                            Slug = "نهبندان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 382,
                            Name = "نیمبلوک",
                            ProvinceId = 10,
                            Slug = "نیمبلوک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 383,
                            Name = "احمدآباد صولت",
                            ProvinceId = 11,
                            Slug = "احمدآباد-صولت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 384,
                            Name = "انابد",
                            ProvinceId = 11,
                            Slug = "انابد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 385,
                            Name = "باجگیران",
                            ProvinceId = 11,
                            Slug = "باجگیران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 386,
                            Name = "باخرز",
                            ProvinceId = 11,
                            Slug = "باخرز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 387,
                            Name = "بار",
                            ProvinceId = 11,
                            Slug = "بار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 388,
                            Name = "بایگ",
                            ProvinceId = 11,
                            Slug = "بایگ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 389,
                            Name = "بجستان",
                            ProvinceId = 11,
                            Slug = "بجستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 390,
                            Name = "بردسکن",
                            ProvinceId = 11,
                            Slug = "بردسکن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 391,
                            Name = "بیدخت",
                            ProvinceId = 11,
                            Slug = "بیدخت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 392,
                            Name = "تایباد",
                            ProvinceId = 11,
                            Slug = "تایباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 393,
                            Name = "تربت جام",
                            ProvinceId = 11,
                            Slug = "تربت-جام",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 394,
                            Name = "تربت حیدریه",
                            ProvinceId = 11,
                            Slug = "تربت-حیدریه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 395,
                            Name = "جغتای",
                            ProvinceId = 11,
                            Slug = "جغتای",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 396,
                            Name = "جنگل",
                            ProvinceId = 11,
                            Slug = "جنگل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 397,
                            Name = "چاپشلو",
                            ProvinceId = 11,
                            Slug = "چاپشلو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 398,
                            Name = "چکنه",
                            ProvinceId = 11,
                            Slug = "چکنه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 399,
                            Name = "چناران",
                            ProvinceId = 11,
                            Slug = "چناران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 400,
                            Name = "خرو",
                            ProvinceId = 11,
                            Slug = "خرو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 401,
                            Name = "خلیل آباد",
                            ProvinceId = 11,
                            Slug = "خلیل-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 402,
                            Name = "خواف",
                            ProvinceId = 11,
                            Slug = "خواف",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 403,
                            Name = "داورزن",
                            ProvinceId = 11,
                            Slug = "داورزن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 404,
                            Name = "درگز",
                            ProvinceId = 11,
                            Slug = "درگز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 405,
                            Name = "در رود",
                            ProvinceId = 11,
                            Slug = "در-رود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 406,
                            Name = "دولت آباد",
                            ProvinceId = 11,
                            Slug = "خراسان-رضوی-دولت-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 407,
                            Name = "رباط سنگ",
                            ProvinceId = 11,
                            Slug = "رباط-سنگ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 408,
                            Name = "رشتخوار",
                            ProvinceId = 11,
                            Slug = "رشتخوار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 409,
                            Name = "رضویه",
                            ProvinceId = 11,
                            Slug = "رضویه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 410,
                            Name = "روداب",
                            ProvinceId = 11,
                            Slug = "روداب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 411,
                            Name = "ریوش",
                            ProvinceId = 11,
                            Slug = "ریوش",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 412,
                            Name = "سبزوار",
                            ProvinceId = 11,
                            Slug = "سبزوار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 413,
                            Name = "سرخس",
                            ProvinceId = 11,
                            Slug = "سرخس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 414,
                            Name = "سفیدسنگ",
                            ProvinceId = 11,
                            Slug = "سفیدسنگ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 415,
                            Name = "سلامی",
                            ProvinceId = 11,
                            Slug = "سلامی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 416,
                            Name = "سلطان آباد",
                            ProvinceId = 11,
                            Slug = "سلطان-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 417,
                            Name = "سنگان",
                            ProvinceId = 11,
                            Slug = "سنگان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 418,
                            Name = "شادمهر",
                            ProvinceId = 11,
                            Slug = "شادمهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 419,
                            Name = "شاندیز",
                            ProvinceId = 11,
                            Slug = "شاندیز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 420,
                            Name = "ششتمد",
                            ProvinceId = 11,
                            Slug = "ششتمد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 421,
                            Name = "شهرآباد",
                            ProvinceId = 11,
                            Slug = "شهرآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 422,
                            Name = "شهرزو",
                            ProvinceId = 11,
                            Slug = "شهرزو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 423,
                            Name = "صالح آباد",
                            ProvinceId = 11,
                            Slug = "خراسان-رضوی-صالح-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 424,
                            Name = "طرقبه",
                            ProvinceId = 11,
                            Slug = "طرقبه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 425,
                            Name = "عشق آباد",
                            ProvinceId = 11,
                            Slug = "خراسان-رضوی-عشق-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 426,
                            Name = "فرهادگرد",
                            ProvinceId = 11,
                            Slug = "فرهادگرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 427,
                            Name = "فریمان",
                            ProvinceId = 11,
                            Slug = "فریمان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 428,
                            Name = "فیروزه",
                            ProvinceId = 11,
                            Slug = "فیروزه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 429,
                            Name = "فیض آباد",
                            ProvinceId = 11,
                            Slug = "فیض-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 430,
                            Name = "قاسم آباد",
                            ProvinceId = 11,
                            Slug = "قاسم-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 431,
                            Name = "قدمگاه",
                            ProvinceId = 11,
                            Slug = "قدمگاه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 432,
                            Name = "قلندرآباد",
                            ProvinceId = 11,
                            Slug = "قلندرآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 433,
                            Name = "قوچان",
                            ProvinceId = 11,
                            Slug = "قوچان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 434,
                            Name = "کاخک",
                            ProvinceId = 11,
                            Slug = "کاخک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 435,
                            Name = "کاریز",
                            ProvinceId = 11,
                            Slug = "کاریز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 436,
                            Name = "کاشمر",
                            ProvinceId = 11,
                            Slug = "کاشمر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 437,
                            Name = "کدکن",
                            ProvinceId = 11,
                            Slug = "کدکن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 438,
                            Name = "کلات",
                            ProvinceId = 11,
                            Slug = "کلات",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 439,
                            Name = "کندر",
                            ProvinceId = 11,
                            Slug = "کندر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 440,
                            Name = "گلمکان",
                            ProvinceId = 11,
                            Slug = "گلمکان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 441,
                            Name = "گناباد",
                            ProvinceId = 11,
                            Slug = "گناباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 442,
                            Name = "لطف آباد",
                            ProvinceId = 11,
                            Slug = "لطف-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 443,
                            Name = "مزدآوند",
                            ProvinceId = 11,
                            Slug = "مزدآوند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 444,
                            Name = "مشهد",
                            ProvinceId = 11,
                            Slug = "مشهد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 445,
                            Name = "ملک آباد",
                            ProvinceId = 11,
                            Slug = "ملک-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 446,
                            Name = "نشتیفان",
                            ProvinceId = 11,
                            Slug = "نشتیفان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 447,
                            Name = "نصرآباد",
                            ProvinceId = 11,
                            Slug = "خراسان-رضوی-نصرآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 448,
                            Name = "نقاب",
                            ProvinceId = 11,
                            Slug = "نقاب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 449,
                            Name = "نوخندان",
                            ProvinceId = 11,
                            Slug = "نوخندان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 450,
                            Name = "نیشابور",
                            ProvinceId = 11,
                            Slug = "نیشابور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 451,
                            Name = "نیل شهر",
                            ProvinceId = 11,
                            Slug = "نیل-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 452,
                            Name = "همت آباد",
                            ProvinceId = 11,
                            Slug = "همت-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 453,
                            Name = "یونسی",
                            ProvinceId = 11,
                            Slug = "یونسی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 454,
                            Name = "اسفراین",
                            ProvinceId = 12,
                            Slug = "اسفراین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 455,
                            Name = "ایور",
                            ProvinceId = 12,
                            Slug = "ایور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 456,
                            Name = "آشخانه",
                            ProvinceId = 12,
                            Slug = "آشخانه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 457,
                            Name = "بجنورد",
                            ProvinceId = 12,
                            Slug = "بجنورد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 458,
                            Name = "پیش قلعه",
                            ProvinceId = 12,
                            Slug = "پیش-قلعه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 459,
                            Name = "تیتکانلو",
                            ProvinceId = 12,
                            Slug = "تیتکانلو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 460,
                            Name = "جاجرم",
                            ProvinceId = 12,
                            Slug = "جاجرم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 461,
                            Name = "حصارگرمخان",
                            ProvinceId = 12,
                            Slug = "حصارگرمخان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 462,
                            Name = "درق",
                            ProvinceId = 12,
                            Slug = "درق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 463,
                            Name = "راز",
                            ProvinceId = 12,
                            Slug = "راز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 464,
                            Name = "سنخواست",
                            ProvinceId = 12,
                            Slug = "سنخواست",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 465,
                            Name = "شوقان",
                            ProvinceId = 12,
                            Slug = "شوقان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 466,
                            Name = "شیروان",
                            ProvinceId = 12,
                            Slug = "شیروان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 467,
                            Name = "صفی آباد",
                            ProvinceId = 12,
                            Slug = "خراسان-شمالی-صفی-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 468,
                            Name = "فاروج",
                            ProvinceId = 12,
                            Slug = "فاروج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 469,
                            Name = "قاضی",
                            ProvinceId = 12,
                            Slug = "قاضی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 470,
                            Name = "گرمه",
                            ProvinceId = 12,
                            Slug = "گرمه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 471,
                            Name = "لوجلی",
                            ProvinceId = 12,
                            Slug = "لوجلی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 472,
                            Name = "اروندکنار",
                            ProvinceId = 13,
                            Slug = "اروندکنار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 473,
                            Name = "الوان",
                            ProvinceId = 13,
                            Slug = "الوان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 474,
                            Name = "امیدیه",
                            ProvinceId = 13,
                            Slug = "امیدیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 475,
                            Name = "اندیمشک",
                            ProvinceId = 13,
                            Slug = "اندیمشک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 476,
                            Name = "اهواز",
                            ProvinceId = 13,
                            Slug = "اهواز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 477,
                            Name = "ایذه",
                            ProvinceId = 13,
                            Slug = "ایذه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 478,
                            Name = "آبادان",
                            ProvinceId = 13,
                            Slug = "آبادان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 479,
                            Name = "آغاجاری",
                            ProvinceId = 13,
                            Slug = "آغاجاری",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 480,
                            Name = "باغ ملک",
                            ProvinceId = 13,
                            Slug = "باغ-ملک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 481,
                            Name = "بستان",
                            ProvinceId = 13,
                            Slug = "بستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 482,
                            Name = "بندرامام خمینی",
                            ProvinceId = 13,
                            Slug = "بندرامام-خمینی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 483,
                            Name = "بندرماهشهر",
                            ProvinceId = 13,
                            Slug = "بندرماهشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 484,
                            Name = "بهبهان",
                            ProvinceId = 13,
                            Slug = "بهبهان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 485,
                            Name = "ترکالکی",
                            ProvinceId = 13,
                            Slug = "ترکالکی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 486,
                            Name = "جایزان",
                            ProvinceId = 13,
                            Slug = "جایزان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 487,
                            Name = "چمران",
                            ProvinceId = 13,
                            Slug = "چمران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 488,
                            Name = "چویبده",
                            ProvinceId = 13,
                            Slug = "چویبده",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 489,
                            Name = "حر",
                            ProvinceId = 13,
                            Slug = "حر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 490,
                            Name = "حسینیه",
                            ProvinceId = 13,
                            Slug = "حسینیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 491,
                            Name = "حمزه",
                            ProvinceId = 13,
                            Slug = "حمزه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 492,
                            Name = "حمیدیه",
                            ProvinceId = 13,
                            Slug = "حمیدیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 493,
                            Name = "خرمشهر",
                            ProvinceId = 13,
                            Slug = "خرمشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 494,
                            Name = "دارخوین",
                            ProvinceId = 13,
                            Slug = "دارخوین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 495,
                            Name = "دزآب",
                            ProvinceId = 13,
                            Slug = "دزآب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 496,
                            Name = "دزفول",
                            ProvinceId = 13,
                            Slug = "دزفول",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 497,
                            Name = "دهدز",
                            ProvinceId = 13,
                            Slug = "دهدز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 498,
                            Name = "رامشیر",
                            ProvinceId = 13,
                            Slug = "رامشیر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 499,
                            Name = "رامهرمز",
                            ProvinceId = 13,
                            Slug = "رامهرمز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 500,
                            Name = "رفیع",
                            ProvinceId = 13,
                            Slug = "رفیع",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 501,
                            Name = "زهره",
                            ProvinceId = 13,
                            Slug = "زهره",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 502,
                            Name = "سالند",
                            ProvinceId = 13,
                            Slug = "سالند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 503,
                            Name = "سردشت",
                            ProvinceId = 13,
                            Slug = "خوزستان-سردشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 504,
                            Name = "سوسنگرد",
                            ProvinceId = 13,
                            Slug = "سوسنگرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 505,
                            Name = "شادگان",
                            ProvinceId = 13,
                            Slug = "شادگان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 506,
                            Name = "شاوور",
                            ProvinceId = 13,
                            Slug = "شاوور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 507,
                            Name = "شرافت",
                            ProvinceId = 13,
                            Slug = "شرافت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 508,
                            Name = "شوش",
                            ProvinceId = 13,
                            Slug = "شوش",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 509,
                            Name = "شوشتر",
                            ProvinceId = 13,
                            Slug = "شوشتر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 510,
                            Name = "شیبان",
                            ProvinceId = 13,
                            Slug = "شیبان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 511,
                            Name = "صالح شهر",
                            ProvinceId = 13,
                            Slug = "صالح-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 512,
                            Name = "صفی آباد",
                            ProvinceId = 13,
                            Slug = "خوزستان-صفی-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 513,
                            Name = "صیدون",
                            ProvinceId = 13,
                            Slug = "صیدون",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 514,
                            Name = "قلعه تل",
                            ProvinceId = 13,
                            Slug = "قلعه-تل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 515,
                            Name = "قلعه خواجه",
                            ProvinceId = 13,
                            Slug = "قلعه-خواجه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 516,
                            Name = "گتوند",
                            ProvinceId = 13,
                            Slug = "گتوند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 517,
                            Name = "لالی",
                            ProvinceId = 13,
                            Slug = "لالی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 518,
                            Name = "مسجدسلیمان",
                            ProvinceId = 13,
                            Slug = "مسجدسلیمان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 520,
                            Name = "ملاثانی",
                            ProvinceId = 13,
                            Slug = "ملاثانی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 521,
                            Name = "میانرود",
                            ProvinceId = 13,
                            Slug = "میانرود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 522,
                            Name = "مینوشهر",
                            ProvinceId = 13,
                            Slug = "مینوشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 523,
                            Name = "هفتگل",
                            ProvinceId = 13,
                            Slug = "هفتگل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 524,
                            Name = "هندیجان",
                            ProvinceId = 13,
                            Slug = "هندیجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 525,
                            Name = "هویزه",
                            ProvinceId = 13,
                            Slug = "هویزه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 526,
                            Name = "ویس",
                            ProvinceId = 13,
                            Slug = "ویس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 527,
                            Name = "ابهر",
                            ProvinceId = 14,
                            Slug = "ابهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 528,
                            Name = "ارمغان خانه",
                            ProvinceId = 14,
                            Slug = "ارمغان-خانه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 529,
                            Name = "آب بر",
                            ProvinceId = 14,
                            Slug = "آب-بر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 530,
                            Name = "چورزق",
                            ProvinceId = 14,
                            Slug = "چورزق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 531,
                            Name = "حلب",
                            ProvinceId = 14,
                            Slug = "حلب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 532,
                            Name = "خرمدره",
                            ProvinceId = 14,
                            Slug = "خرمدره",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 533,
                            Name = "دندی",
                            ProvinceId = 14,
                            Slug = "دندی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 534,
                            Name = "زرین آباد",
                            ProvinceId = 14,
                            Slug = "زرین-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 535,
                            Name = "زرین رود",
                            ProvinceId = 14,
                            Slug = "زرین-رود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 536,
                            Name = "زنجان",
                            ProvinceId = 14,
                            Slug = "زنجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 537,
                            Name = "سجاس",
                            ProvinceId = 14,
                            Slug = "سجاس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 538,
                            Name = "سلطانیه",
                            ProvinceId = 14,
                            Slug = "سلطانیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 539,
                            Name = "سهرورد",
                            ProvinceId = 14,
                            Slug = "سهرورد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 540,
                            Name = "صائین قلعه",
                            ProvinceId = 14,
                            Slug = "صائین-قلعه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 541,
                            Name = "قیدار",
                            ProvinceId = 14,
                            Slug = "قیدار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 542,
                            Name = "گرماب",
                            ProvinceId = 14,
                            Slug = "گرماب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 543,
                            Name = "ماه نشان",
                            ProvinceId = 14,
                            Slug = "ماه-نشان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 544,
                            Name = "هیدج",
                            ProvinceId = 14,
                            Slug = "هیدج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 545,
                            Name = "امیریه",
                            ProvinceId = 15,
                            Slug = "امیریه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 546,
                            Name = "ایوانکی",
                            ProvinceId = 15,
                            Slug = "ایوانکی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 547,
                            Name = "آرادان",
                            ProvinceId = 15,
                            Slug = "آرادان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 548,
                            Name = "بسطام",
                            ProvinceId = 15,
                            Slug = "بسطام",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 549,
                            Name = "بیارجمند",
                            ProvinceId = 15,
                            Slug = "بیارجمند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 550,
                            Name = "دامغان",
                            ProvinceId = 15,
                            Slug = "دامغان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 551,
                            Name = "درجزین",
                            ProvinceId = 15,
                            Slug = "درجزین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 552,
                            Name = "دیباج",
                            ProvinceId = 15,
                            Slug = "دیباج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 553,
                            Name = "سرخه",
                            ProvinceId = 15,
                            Slug = "سرخه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 554,
                            Name = "سمنان",
                            ProvinceId = 15,
                            Slug = "سمنان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 555,
                            Name = "شاهرود",
                            ProvinceId = 15,
                            Slug = "شاهرود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 556,
                            Name = "شهمیرزاد",
                            ProvinceId = 15,
                            Slug = "شهمیرزاد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 557,
                            Name = "کلاته خیج",
                            ProvinceId = 15,
                            Slug = "کلاته-خیج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 558,
                            Name = "گرمسار",
                            ProvinceId = 15,
                            Slug = "گرمسار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 559,
                            Name = "مجن",
                            ProvinceId = 15,
                            Slug = "مجن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 560,
                            Name = "مهدی شهر",
                            ProvinceId = 15,
                            Slug = "مهدی-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 561,
                            Name = "میامی",
                            ProvinceId = 15,
                            Slug = "میامی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 562,
                            Name = "ادیمی",
                            ProvinceId = 16,
                            Slug = "ادیمی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 563,
                            Name = "اسپکه",
                            ProvinceId = 16,
                            Slug = "اسپکه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 564,
                            Name = "ایرانشهر",
                            ProvinceId = 16,
                            Slug = "ایرانشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 565,
                            Name = "بزمان",
                            ProvinceId = 16,
                            Slug = "بزمان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 566,
                            Name = "بمپور",
                            ProvinceId = 16,
                            Slug = "بمپور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 567,
                            Name = "بنت",
                            ProvinceId = 16,
                            Slug = "بنت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 568,
                            Name = "بنجار",
                            ProvinceId = 16,
                            Slug = "بنجار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 569,
                            Name = "پیشین",
                            ProvinceId = 16,
                            Slug = "پیشین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 570,
                            Name = "جالق",
                            ProvinceId = 16,
                            Slug = "جالق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 571,
                            Name = "چابهار",
                            ProvinceId = 16,
                            Slug = "چابهار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 572,
                            Name = "خاش",
                            ProvinceId = 16,
                            Slug = "خاش",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 573,
                            Name = "دوست محمد",
                            ProvinceId = 16,
                            Slug = "دوست-محمد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 574,
                            Name = "راسک",
                            ProvinceId = 16,
                            Slug = "راسک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 575,
                            Name = "زابل",
                            ProvinceId = 16,
                            Slug = "زابل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 576,
                            Name = "زابلی",
                            ProvinceId = 16,
                            Slug = "زابلی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 577,
                            Name = "زاهدان",
                            ProvinceId = 16,
                            Slug = "زاهدان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 578,
                            Name = "زهک",
                            ProvinceId = 16,
                            Slug = "زهک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 579,
                            Name = "سراوان",
                            ProvinceId = 16,
                            Slug = "سراوان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 580,
                            Name = "سرباز",
                            ProvinceId = 16,
                            Slug = "سرباز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 581,
                            Name = "سوران",
                            ProvinceId = 16,
                            Slug = "سوران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 582,
                            Name = "سیرکان",
                            ProvinceId = 16,
                            Slug = "سیرکان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 583,
                            Name = "علی اکبر",
                            ProvinceId = 16,
                            Slug = "علی-اکبر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 584,
                            Name = "فنوج",
                            ProvinceId = 16,
                            Slug = "فنوج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 585,
                            Name = "قصرقند",
                            ProvinceId = 16,
                            Slug = "قصرقند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 586,
                            Name = "کنارک",
                            ProvinceId = 16,
                            Slug = "کنارک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 587,
                            Name = "گشت",
                            ProvinceId = 16,
                            Slug = "گشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 588,
                            Name = "گلمورتی",
                            ProvinceId = 16,
                            Slug = "گلمورتی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 589,
                            Name = "محمدان",
                            ProvinceId = 16,
                            Slug = "محمدان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 590,
                            Name = "محمدآباد",
                            ProvinceId = 16,
                            Slug = "سیستان-و-بلوچستان-محمدآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 591,
                            Name = "محمدی",
                            ProvinceId = 16,
                            Slug = "محمدی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 592,
                            Name = "میرجاوه",
                            ProvinceId = 16,
                            Slug = "میرجاوه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 593,
                            Name = "نصرت آباد",
                            ProvinceId = 16,
                            Slug = "نصرت-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 594,
                            Name = "نگور",
                            ProvinceId = 16,
                            Slug = "نگور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 595,
                            Name = "نوک آباد",
                            ProvinceId = 16,
                            Slug = "نوک-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 596,
                            Name = "نیک شهر",
                            ProvinceId = 16,
                            Slug = "نیک-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 597,
                            Name = "هیدوچ",
                            ProvinceId = 16,
                            Slug = "هیدوچ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 598,
                            Name = "اردکان",
                            ProvinceId = 17,
                            Slug = "فارس-اردکان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 599,
                            Name = "ارسنجان",
                            ProvinceId = 17,
                            Slug = "ارسنجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 600,
                            Name = "استهبان",
                            ProvinceId = 17,
                            Slug = "استهبان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 601,
                            Name = "اشکنان",
                            ProvinceId = 17,
                            Slug = "اشکنان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 602,
                            Name = "افزر",
                            ProvinceId = 17,
                            Slug = "افزر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 603,
                            Name = "اقلید",
                            ProvinceId = 17,
                            Slug = "اقلید",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 604,
                            Name = "امام شهر",
                            ProvinceId = 17,
                            Slug = "امام-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 605,
                            Name = "اهل",
                            ProvinceId = 17,
                            Slug = "اهل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 606,
                            Name = "اوز",
                            ProvinceId = 17,
                            Slug = "اوز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 607,
                            Name = "ایج",
                            ProvinceId = 17,
                            Slug = "ایج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 608,
                            Name = "ایزدخواست",
                            ProvinceId = 17,
                            Slug = "ایزدخواست",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 609,
                            Name = "آباده",
                            ProvinceId = 17,
                            Slug = "آباده",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 610,
                            Name = "آباده طشک",
                            ProvinceId = 17,
                            Slug = "آباده-طشک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 611,
                            Name = "باب انار",
                            ProvinceId = 17,
                            Slug = "باب-انار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 612,
                            Name = "بالاده",
                            ProvinceId = 17,
                            Slug = "فارس-بالاده",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 613,
                            Name = "بنارویه",
                            ProvinceId = 17,
                            Slug = "بنارویه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 614,
                            Name = "بهمن",
                            ProvinceId = 17,
                            Slug = "بهمن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 615,
                            Name = "بوانات",
                            ProvinceId = 17,
                            Slug = "بوانات",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 616,
                            Name = "بیرم",
                            ProvinceId = 17,
                            Slug = "بیرم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 617,
                            Name = "بیضا",
                            ProvinceId = 17,
                            Slug = "بیضا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 618,
                            Name = "جنت شهر",
                            ProvinceId = 17,
                            Slug = "جنت-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 619,
                            Name = "جهرم",
                            ProvinceId = 17,
                            Slug = "جهرم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 620,
                            Name = "جویم",
                            ProvinceId = 17,
                            Slug = "جویم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 621,
                            Name = "زرین دشت",
                            ProvinceId = 17,
                            Slug = "زرین-دشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 622,
                            Name = "حسن آباد",
                            ProvinceId = 17,
                            Slug = "فارس-حسن-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 623,
                            Name = "خان زنیان",
                            ProvinceId = 17,
                            Slug = "خان-زنیان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 624,
                            Name = "خاوران",
                            ProvinceId = 17,
                            Slug = "خاوران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 625,
                            Name = "خرامه",
                            ProvinceId = 17,
                            Slug = "خرامه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 626,
                            Name = "خشت",
                            ProvinceId = 17,
                            Slug = "خشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 627,
                            Name = "خنج",
                            ProvinceId = 17,
                            Slug = "خنج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 628,
                            Name = "خور",
                            ProvinceId = 17,
                            Slug = "فارس-خور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 629,
                            Name = "داراب",
                            ProvinceId = 17,
                            Slug = "داراب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 630,
                            Name = "داریان",
                            ProvinceId = 17,
                            Slug = "داریان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 631,
                            Name = "دبیران",
                            ProvinceId = 17,
                            Slug = "دبیران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 632,
                            Name = "دژکرد",
                            ProvinceId = 17,
                            Slug = "دژکرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 633,
                            Name = "دهرم",
                            ProvinceId = 17,
                            Slug = "دهرم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 634,
                            Name = "دوبرجی",
                            ProvinceId = 17,
                            Slug = "دوبرجی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 635,
                            Name = "رامجرد",
                            ProvinceId = 17,
                            Slug = "رامجرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 636,
                            Name = "رونیز",
                            ProvinceId = 17,
                            Slug = "رونیز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 637,
                            Name = "زاهدشهر",
                            ProvinceId = 17,
                            Slug = "زاهدشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 638,
                            Name = "زرقان",
                            ProvinceId = 17,
                            Slug = "زرقان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 639,
                            Name = "سده",
                            ProvinceId = 17,
                            Slug = "سده",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 640,
                            Name = "سروستان",
                            ProvinceId = 17,
                            Slug = "سروستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 641,
                            Name = "سعادت شهر",
                            ProvinceId = 17,
                            Slug = "سعادت-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 642,
                            Name = "سورمق",
                            ProvinceId = 17,
                            Slug = "سورمق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 643,
                            Name = "سیدان",
                            ProvinceId = 17,
                            Slug = "سیدان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 644,
                            Name = "ششده",
                            ProvinceId = 17,
                            Slug = "ششده",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 645,
                            Name = "شهرپیر",
                            ProvinceId = 17,
                            Slug = "شهرپیر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 646,
                            Name = "شهرصدرا",
                            ProvinceId = 17,
                            Slug = "شهرصدرا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 647,
                            Name = "شیراز",
                            ProvinceId = 17,
                            Slug = "شیراز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 648,
                            Name = "صغاد",
                            ProvinceId = 17,
                            Slug = "صغاد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 649,
                            Name = "صفاشهر",
                            ProvinceId = 17,
                            Slug = "صفاشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 650,
                            Name = "علامرودشت",
                            ProvinceId = 17,
                            Slug = "علامرودشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 651,
                            Name = "فدامی",
                            ProvinceId = 17,
                            Slug = "فدامی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 652,
                            Name = "فراشبند",
                            ProvinceId = 17,
                            Slug = "فراشبند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 653,
                            Name = "فسا",
                            ProvinceId = 17,
                            Slug = "فسا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 654,
                            Name = "فیروزآباد",
                            ProvinceId = 17,
                            Slug = "فارس-فیروزآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 655,
                            Name = "قائمیه",
                            ProvinceId = 17,
                            Slug = "قائمیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 656,
                            Name = "قادرآباد",
                            ProvinceId = 17,
                            Slug = "قادرآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 657,
                            Name = "قطب آباد",
                            ProvinceId = 17,
                            Slug = "قطب-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 658,
                            Name = "قطرویه",
                            ProvinceId = 17,
                            Slug = "قطرویه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 659,
                            Name = "قیر",
                            ProvinceId = 17,
                            Slug = "قیر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 660,
                            Name = "کارزین (فتح آباد)",
                            ProvinceId = 17,
                            Slug = "کارزین-فتح-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 661,
                            Name = "کازرون",
                            ProvinceId = 17,
                            Slug = "کازرون",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 662,
                            Name = "کامفیروز",
                            ProvinceId = 17,
                            Slug = "کامفیروز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 663,
                            Name = "کره ای",
                            ProvinceId = 17,
                            Slug = "کره-ای",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 664,
                            Name = "کنارتخته",
                            ProvinceId = 17,
                            Slug = "کنارتخته",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 665,
                            Name = "کوار",
                            ProvinceId = 17,
                            Slug = "کوار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 666,
                            Name = "گراش",
                            ProvinceId = 17,
                            Slug = "گراش",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 667,
                            Name = "گله دار",
                            ProvinceId = 17,
                            Slug = "گله-دار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 668,
                            Name = "لار",
                            ProvinceId = 17,
                            Slug = "لار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 669,
                            Name = "لامرد",
                            ProvinceId = 17,
                            Slug = "لامرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 670,
                            Name = "لپویی",
                            ProvinceId = 17,
                            Slug = "لپویی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 671,
                            Name = "لطیفی",
                            ProvinceId = 17,
                            Slug = "لطیفی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 672,
                            Name = "مبارک آباددیز",
                            ProvinceId = 17,
                            Slug = "مبارک-آباددیز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 673,
                            Name = "مرودشت",
                            ProvinceId = 17,
                            Slug = "مرودشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 674,
                            Name = "مشکان",
                            ProvinceId = 17,
                            Slug = "مشکان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 675,
                            Name = "مصیری",
                            ProvinceId = 17,
                            Slug = "مصیری",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 676,
                            Name = "مهر",
                            ProvinceId = 17,
                            Slug = "مهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 677,
                            Name = "میمند",
                            ProvinceId = 17,
                            Slug = "میمند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 678,
                            Name = "نوبندگان",
                            ProvinceId = 17,
                            Slug = "نوبندگان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 679,
                            Name = "نوجین",
                            ProvinceId = 17,
                            Slug = "نوجین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 680,
                            Name = "نودان",
                            ProvinceId = 17,
                            Slug = "نودان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 681,
                            Name = "نورآباد",
                            ProvinceId = 17,
                            Slug = "فارس-نورآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 682,
                            Name = "نی ریز",
                            ProvinceId = 17,
                            Slug = "نی-ریز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 683,
                            Name = "وراوی",
                            ProvinceId = 17,
                            Slug = "وراوی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 684,
                            Name = "ارداق",
                            ProvinceId = 18,
                            Slug = "ارداق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 685,
                            Name = "اسفرورین",
                            ProvinceId = 18,
                            Slug = "اسفرورین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 686,
                            Name = "اقبالیه",
                            ProvinceId = 18,
                            Slug = "اقبالیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 687,
                            Name = "الوند",
                            ProvinceId = 18,
                            Slug = "الوند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 688,
                            Name = "آبگرم",
                            ProvinceId = 18,
                            Slug = "آبگرم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 689,
                            Name = "آبیک",
                            ProvinceId = 18,
                            Slug = "آبیک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 690,
                            Name = "آوج",
                            ProvinceId = 18,
                            Slug = "آوج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 691,
                            Name = "بوئین زهرا",
                            ProvinceId = 18,
                            Slug = "بوئین-زهرا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 692,
                            Name = "بیدستان",
                            ProvinceId = 18,
                            Slug = "بیدستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 693,
                            Name = "تاکستان",
                            ProvinceId = 18,
                            Slug = "تاکستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 694,
                            Name = "خاکعلی",
                            ProvinceId = 18,
                            Slug = "خاکعلی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 695,
                            Name = "خرمدشت",
                            ProvinceId = 18,
                            Slug = "خرمدشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 696,
                            Name = "دانسفهان",
                            ProvinceId = 18,
                            Slug = "دانسفهان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 697,
                            Name = "رازمیان",
                            ProvinceId = 18,
                            Slug = "رازمیان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 698,
                            Name = "سگزآباد",
                            ProvinceId = 18,
                            Slug = "سگزآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 699,
                            Name = "سیردان",
                            ProvinceId = 18,
                            Slug = "سیردان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 700,
                            Name = "شال",
                            ProvinceId = 18,
                            Slug = "شال",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 701,
                            Name = "شریفیه",
                            ProvinceId = 18,
                            Slug = "شریفیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 702,
                            Name = "ضیاآباد",
                            ProvinceId = 18,
                            Slug = "ضیاآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 703,
                            Name = "قزوین",
                            ProvinceId = 18,
                            Slug = "قزوین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 704,
                            Name = "کوهین",
                            ProvinceId = 18,
                            Slug = "کوهین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 705,
                            Name = "محمدیه",
                            ProvinceId = 18,
                            Slug = "محمدیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 706,
                            Name = "محمودآباد نمونه",
                            ProvinceId = 18,
                            Slug = "محمودآباد-نمونه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 707,
                            Name = "معلم کلایه",
                            ProvinceId = 18,
                            Slug = "معلم-کلایه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 708,
                            Name = "نرجه",
                            ProvinceId = 18,
                            Slug = "نرجه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 709,
                            Name = "جعفریه",
                            ProvinceId = 19,
                            Slug = "جعفریه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 710,
                            Name = "دستجرد",
                            ProvinceId = 19,
                            Slug = "دستجرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 711,
                            Name = "سلفچگان",
                            ProvinceId = 19,
                            Slug = "سلفچگان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 712,
                            Name = "قم",
                            ProvinceId = 19,
                            Slug = "قم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 713,
                            Name = "قنوات",
                            ProvinceId = 19,
                            Slug = "قنوات",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 714,
                            Name = "کهک",
                            ProvinceId = 19,
                            Slug = "کهک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 715,
                            Name = "آرمرده",
                            ProvinceId = 20,
                            Slug = "آرمرده",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 716,
                            Name = "بابارشانی",
                            ProvinceId = 20,
                            Slug = "بابارشانی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 717,
                            Name = "بانه",
                            ProvinceId = 20,
                            Slug = "بانه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 718,
                            Name = "بلبان آباد",
                            ProvinceId = 20,
                            Slug = "بلبان-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 719,
                            Name = "بوئین سفلی",
                            ProvinceId = 20,
                            Slug = "بوئین-سفلی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 720,
                            Name = "بیجار",
                            ProvinceId = 20,
                            Slug = "بیجار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 721,
                            Name = "چناره",
                            ProvinceId = 20,
                            Slug = "چناره",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 722,
                            Name = "دزج",
                            ProvinceId = 20,
                            Slug = "دزج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 723,
                            Name = "دلبران",
                            ProvinceId = 20,
                            Slug = "دلبران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 724,
                            Name = "دهگلان",
                            ProvinceId = 20,
                            Slug = "دهگلان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 725,
                            Name = "دیواندره",
                            ProvinceId = 20,
                            Slug = "دیواندره",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 726,
                            Name = "زرینه",
                            ProvinceId = 20,
                            Slug = "زرینه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 727,
                            Name = "سروآباد",
                            ProvinceId = 20,
                            Slug = "سروآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 728,
                            Name = "سریش آباد",
                            ProvinceId = 20,
                            Slug = "سریش-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 729,
                            Name = "سقز",
                            ProvinceId = 20,
                            Slug = "سقز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 730,
                            Name = "سنندج",
                            ProvinceId = 20,
                            Slug = "سنندج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 731,
                            Name = "شویشه",
                            ProvinceId = 20,
                            Slug = "شویشه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 732,
                            Name = "صاحب",
                            ProvinceId = 20,
                            Slug = "صاحب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 733,
                            Name = "قروه",
                            ProvinceId = 20,
                            Slug = "قروه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 734,
                            Name = "کامیاران",
                            ProvinceId = 20,
                            Slug = "کامیاران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 735,
                            Name = "کانی دینار",
                            ProvinceId = 20,
                            Slug = "کانی-دینار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 736,
                            Name = "کانی سور",
                            ProvinceId = 20,
                            Slug = "کانی-سور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 737,
                            Name = "مریوان",
                            ProvinceId = 20,
                            Slug = "مریوان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 738,
                            Name = "موچش",
                            ProvinceId = 20,
                            Slug = "موچش",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 739,
                            Name = "یاسوکند",
                            ProvinceId = 20,
                            Slug = "یاسوکند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 740,
                            Name = "اختیارآباد",
                            ProvinceId = 21,
                            Slug = "اختیارآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 741,
                            Name = "ارزوئیه",
                            ProvinceId = 21,
                            Slug = "ارزوئیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 742,
                            Name = "امین شهر",
                            ProvinceId = 21,
                            Slug = "امین-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 743,
                            Name = "انار",
                            ProvinceId = 21,
                            Slug = "انار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 744,
                            Name = "اندوهجرد",
                            ProvinceId = 21,
                            Slug = "اندوهجرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 745,
                            Name = "باغین",
                            ProvinceId = 21,
                            Slug = "باغین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 746,
                            Name = "بافت",
                            ProvinceId = 21,
                            Slug = "بافت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 747,
                            Name = "بردسیر",
                            ProvinceId = 21,
                            Slug = "بردسیر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 748,
                            Name = "بروات",
                            ProvinceId = 21,
                            Slug = "بروات",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 749,
                            Name = "بزنجان",
                            ProvinceId = 21,
                            Slug = "بزنجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 750,
                            Name = "بم",
                            ProvinceId = 21,
                            Slug = "بم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 751,
                            Name = "بهرمان",
                            ProvinceId = 21,
                            Slug = "بهرمان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 752,
                            Name = "پاریز",
                            ProvinceId = 21,
                            Slug = "پاریز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 753,
                            Name = "جبالبارز",
                            ProvinceId = 21,
                            Slug = "جبالبارز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 754,
                            Name = "جوپار",
                            ProvinceId = 21,
                            Slug = "جوپار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 755,
                            Name = "جوزم",
                            ProvinceId = 21,
                            Slug = "جوزم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 756,
                            Name = "جیرفت",
                            ProvinceId = 21,
                            Slug = "جیرفت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 757,
                            Name = "چترود",
                            ProvinceId = 21,
                            Slug = "چترود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 758,
                            Name = "خاتون آباد",
                            ProvinceId = 21,
                            Slug = "خاتون-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 759,
                            Name = "خانوک",
                            ProvinceId = 21,
                            Slug = "خانوک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 760,
                            Name = "خورسند",
                            ProvinceId = 21,
                            Slug = "خورسند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 761,
                            Name = "درب بهشت",
                            ProvinceId = 21,
                            Slug = "درب-بهشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 762,
                            Name = "دهج",
                            ProvinceId = 21,
                            Slug = "دهج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 763,
                            Name = "رابر",
                            ProvinceId = 21,
                            Slug = "رابر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 764,
                            Name = "راور",
                            ProvinceId = 21,
                            Slug = "راور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 765,
                            Name = "راین",
                            ProvinceId = 21,
                            Slug = "راین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 766,
                            Name = "رفسنجان",
                            ProvinceId = 21,
                            Slug = "رفسنجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 767,
                            Name = "رودبار",
                            ProvinceId = 21,
                            Slug = "کرمان-رودبار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 768,
                            Name = "ریحان شهر",
                            ProvinceId = 21,
                            Slug = "ریحان-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 769,
                            Name = "زرند",
                            ProvinceId = 21,
                            Slug = "زرند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 770,
                            Name = "زنگی آباد",
                            ProvinceId = 21,
                            Slug = "زنگی-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 771,
                            Name = "زیدآباد",
                            ProvinceId = 21,
                            Slug = "زیدآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 772,
                            Name = "سیرجان",
                            ProvinceId = 21,
                            Slug = "سیرجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 773,
                            Name = "شهداد",
                            ProvinceId = 21,
                            Slug = "شهداد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 774,
                            Name = "شهربابک",
                            ProvinceId = 21,
                            Slug = "شهربابک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 775,
                            Name = "صفائیه",
                            ProvinceId = 21,
                            Slug = "صفائیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 776,
                            Name = "عنبرآباد",
                            ProvinceId = 21,
                            Slug = "عنبرآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 777,
                            Name = "فاریاب",
                            ProvinceId = 21,
                            Slug = "فاریاب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 778,
                            Name = "فهرج",
                            ProvinceId = 21,
                            Slug = "فهرج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 779,
                            Name = "قلعه گنج",
                            ProvinceId = 21,
                            Slug = "قلعه-گنج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 780,
                            Name = "کاظم آباد",
                            ProvinceId = 21,
                            Slug = "کاظم-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 781,
                            Name = "کرمان",
                            ProvinceId = 21,
                            Slug = "کرمان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 782,
                            Name = "کشکوئیه",
                            ProvinceId = 21,
                            Slug = "کشکوئیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 783,
                            Name = "کهنوج",
                            ProvinceId = 21,
                            Slug = "کهنوج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 784,
                            Name = "کوهبنان",
                            ProvinceId = 21,
                            Slug = "کوهبنان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 785,
                            Name = "کیانشهر",
                            ProvinceId = 21,
                            Slug = "کیانشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 786,
                            Name = "گلباف",
                            ProvinceId = 21,
                            Slug = "گلباف",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 787,
                            Name = "گلزار",
                            ProvinceId = 21,
                            Slug = "گلزار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 788,
                            Name = "لاله زار",
                            ProvinceId = 21,
                            Slug = "لاله-زار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 789,
                            Name = "ماهان",
                            ProvinceId = 21,
                            Slug = "ماهان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 790,
                            Name = "محمدآباد",
                            ProvinceId = 21,
                            Slug = "کرمان-محمدآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 791,
                            Name = "محی آباد",
                            ProvinceId = 21,
                            Slug = "محی-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 792,
                            Name = "مردهک",
                            ProvinceId = 21,
                            Slug = "مردهک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 793,
                            Name = "مس سرچشمه",
                            ProvinceId = 21,
                            Slug = "مس-سرچشمه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 794,
                            Name = "منوجان",
                            ProvinceId = 21,
                            Slug = "منوجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 795,
                            Name = "نجف شهر",
                            ProvinceId = 21,
                            Slug = "نجف-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 796,
                            Name = "نرماشیر",
                            ProvinceId = 21,
                            Slug = "نرماشیر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 797,
                            Name = "نظام شهر",
                            ProvinceId = 21,
                            Slug = "نظام-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 798,
                            Name = "نگار",
                            ProvinceId = 21,
                            Slug = "نگار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 799,
                            Name = "نودژ",
                            ProvinceId = 21,
                            Slug = "نودژ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 800,
                            Name = "هجدک",
                            ProvinceId = 21,
                            Slug = "هجدک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 801,
                            Name = "یزدان شهر",
                            ProvinceId = 21,
                            Slug = "یزدان-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 802,
                            Name = "ازگله",
                            ProvinceId = 22,
                            Slug = "ازگله",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 803,
                            Name = "اسلام آباد غرب",
                            ProvinceId = 22,
                            Slug = "اسلام-آباد-غرب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 804,
                            Name = "باینگان",
                            ProvinceId = 22,
                            Slug = "باینگان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 805,
                            Name = "بیستون",
                            ProvinceId = 22,
                            Slug = "بیستون",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 806,
                            Name = "پاوه",
                            ProvinceId = 22,
                            Slug = "پاوه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 807,
                            Name = "تازه آباد",
                            ProvinceId = 22,
                            Slug = "تازه-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 808,
                            Name = "جوان رود",
                            ProvinceId = 22,
                            Slug = "جوان-رود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 809,
                            Name = "حمیل",
                            ProvinceId = 22,
                            Slug = "حمیل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 810,
                            Name = "ماهیدشت",
                            ProvinceId = 22,
                            Slug = "ماهیدشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 811,
                            Name = "روانسر",
                            ProvinceId = 22,
                            Slug = "روانسر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 812,
                            Name = "سرپل ذهاب",
                            ProvinceId = 22,
                            Slug = "سرپل-ذهاب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 813,
                            Name = "سرمست",
                            ProvinceId = 22,
                            Slug = "سرمست",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 814,
                            Name = "سطر",
                            ProvinceId = 22,
                            Slug = "سطر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 815,
                            Name = "سنقر",
                            ProvinceId = 22,
                            Slug = "سنقر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 816,
                            Name = "سومار",
                            ProvinceId = 22,
                            Slug = "سومار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 817,
                            Name = "شاهو",
                            ProvinceId = 22,
                            Slug = "شاهو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 818,
                            Name = "صحنه",
                            ProvinceId = 22,
                            Slug = "صحنه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 819,
                            Name = "قصرشیرین",
                            ProvinceId = 22,
                            Slug = "قصرشیرین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 820,
                            Name = "کرمانشاه",
                            ProvinceId = 22,
                            Slug = "کرمانشاه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 821,
                            Name = "کرندغرب",
                            ProvinceId = 22,
                            Slug = "کرندغرب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 822,
                            Name = "کنگاور",
                            ProvinceId = 22,
                            Slug = "کنگاور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 823,
                            Name = "کوزران",
                            ProvinceId = 22,
                            Slug = "کوزران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 824,
                            Name = "گهواره",
                            ProvinceId = 22,
                            Slug = "گهواره",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 825,
                            Name = "گیلانغرب",
                            ProvinceId = 22,
                            Slug = "گیلانغرب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 826,
                            Name = "میان راهان",
                            ProvinceId = 22,
                            Slug = "میان-راهان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 827,
                            Name = "نودشه",
                            ProvinceId = 22,
                            Slug = "نودشه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 828,
                            Name = "نوسود",
                            ProvinceId = 22,
                            Slug = "نوسود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 829,
                            Name = "هرسین",
                            ProvinceId = 22,
                            Slug = "هرسین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 830,
                            Name = "هلشی",
                            ProvinceId = 22,
                            Slug = "هلشی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 831,
                            Name = "باشت",
                            ProvinceId = 23,
                            Slug = "باشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 832,
                            Name = "پاتاوه",
                            ProvinceId = 23,
                            Slug = "پاتاوه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 833,
                            Name = "چرام",
                            ProvinceId = 23,
                            Slug = "چرام",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 834,
                            Name = "چیتاب",
                            ProvinceId = 23,
                            Slug = "چیتاب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 835,
                            Name = "دهدشت",
                            ProvinceId = 23,
                            Slug = "دهدشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 836,
                            Name = "دوگنبدان",
                            ProvinceId = 23,
                            Slug = "دوگنبدان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 837,
                            Name = "دیشموک",
                            ProvinceId = 23,
                            Slug = "دیشموک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 838,
                            Name = "سوق",
                            ProvinceId = 23,
                            Slug = "سوق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 839,
                            Name = "سی سخت",
                            ProvinceId = 23,
                            Slug = "سی-سخت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 840,
                            Name = "قلعه رئیسی",
                            ProvinceId = 23,
                            Slug = "قلعه-رئیسی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 841,
                            Name = "گراب سفلی",
                            ProvinceId = 23,
                            Slug = "گراب-سفلی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 842,
                            Name = "لنده",
                            ProvinceId = 23,
                            Slug = "لنده",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 843,
                            Name = "لیکک",
                            ProvinceId = 23,
                            Slug = "لیکک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 844,
                            Name = "مادوان",
                            ProvinceId = 23,
                            Slug = "مادوان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 845,
                            Name = "مارگون",
                            ProvinceId = 23,
                            Slug = "مارگون",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 846,
                            Name = "یاسوج",
                            ProvinceId = 23,
                            Slug = "یاسوج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 847,
                            Name = "انبارآلوم",
                            ProvinceId = 24,
                            Slug = "انبارآلوم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 848,
                            Name = "اینچه برون",
                            ProvinceId = 24,
                            Slug = "اینچه-برون",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 849,
                            Name = "آزادشهر",
                            ProvinceId = 24,
                            Slug = "آزادشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 850,
                            Name = "آق قلا",
                            ProvinceId = 24,
                            Slug = "آق-قلا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 851,
                            Name = "بندرترکمن",
                            ProvinceId = 24,
                            Slug = "بندرترکمن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 852,
                            Name = "بندرگز",
                            ProvinceId = 24,
                            Slug = "بندرگز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 853,
                            Name = "جلین",
                            ProvinceId = 24,
                            Slug = "جلین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 854,
                            Name = "خان ببین",
                            ProvinceId = 24,
                            Slug = "خان-ببین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 855,
                            Name = "دلند",
                            ProvinceId = 24,
                            Slug = "دلند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 856,
                            Name = "رامیان",
                            ProvinceId = 24,
                            Slug = "رامیان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 857,
                            Name = "سرخنکلاته",
                            ProvinceId = 24,
                            Slug = "سرخنکلاته",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 858,
                            Name = "سیمین شهر",
                            ProvinceId = 24,
                            Slug = "سیمین-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 859,
                            Name = "علی آباد کتول",
                            ProvinceId = 24,
                            Slug = "علی-آباد-کتول",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 860,
                            Name = "فاضل آباد",
                            ProvinceId = 24,
                            Slug = "فاضل-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 861,
                            Name = "کردکوی",
                            ProvinceId = 24,
                            Slug = "کردکوی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 862,
                            Name = "کلاله",
                            ProvinceId = 24,
                            Slug = "کلاله",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 863,
                            Name = "گالیکش",
                            ProvinceId = 24,
                            Slug = "گالیکش",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 864,
                            Name = "گرگان",
                            ProvinceId = 24,
                            Slug = "گرگان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 865,
                            Name = "گمیش تپه",
                            ProvinceId = 24,
                            Slug = "گمیش-تپه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 866,
                            Name = "گنبدکاووس",
                            ProvinceId = 24,
                            Slug = "گنبدکاووس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 867,
                            Name = "مراوه",
                            ProvinceId = 24,
                            Slug = "مراوه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 868,
                            Name = "مینودشت",
                            ProvinceId = 24,
                            Slug = "مینودشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 869,
                            Name = "نگین شهر",
                            ProvinceId = 24,
                            Slug = "نگین-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 870,
                            Name = "نوده خاندوز",
                            ProvinceId = 24,
                            Slug = "نوده-خاندوز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 871,
                            Name = "نوکنده",
                            ProvinceId = 24,
                            Slug = "نوکنده",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 872,
                            Name = "ازنا",
                            ProvinceId = 25,
                            Slug = "ازنا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 873,
                            Name = "اشترینان",
                            ProvinceId = 25,
                            Slug = "اشترینان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 874,
                            Name = "الشتر",
                            ProvinceId = 25,
                            Slug = "الشتر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 875,
                            Name = "الیگودرز",
                            ProvinceId = 25,
                            Slug = "الیگودرز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 876,
                            Name = "بروجرد",
                            ProvinceId = 25,
                            Slug = "بروجرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 877,
                            Name = "پلدختر",
                            ProvinceId = 25,
                            Slug = "پلدختر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 878,
                            Name = "چالانچولان",
                            ProvinceId = 25,
                            Slug = "چالانچولان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 879,
                            Name = "چغلوندی",
                            ProvinceId = 25,
                            Slug = "چغلوندی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 880,
                            Name = "چقابل",
                            ProvinceId = 25,
                            Slug = "چقابل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 881,
                            Name = "خرم آباد",
                            ProvinceId = 25,
                            Slug = "لرستان-خرم-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 882,
                            Name = "درب گنبد",
                            ProvinceId = 25,
                            Slug = "درب-گنبد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 883,
                            Name = "دورود",
                            ProvinceId = 25,
                            Slug = "دورود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 884,
                            Name = "زاغه",
                            ProvinceId = 25,
                            Slug = "زاغه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 885,
                            Name = "سپیددشت",
                            ProvinceId = 25,
                            Slug = "سپیددشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 886,
                            Name = "سراب دوره",
                            ProvinceId = 25,
                            Slug = "سراب-دوره",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 887,
                            Name = "فیروزآباد",
                            ProvinceId = 25,
                            Slug = "لرستان-فیروزآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 888,
                            Name = "کونانی",
                            ProvinceId = 25,
                            Slug = "کونانی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 889,
                            Name = "کوهدشت",
                            ProvinceId = 25,
                            Slug = "کوهدشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 890,
                            Name = "گراب",
                            ProvinceId = 25,
                            Slug = "گراب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 891,
                            Name = "معمولان",
                            ProvinceId = 25,
                            Slug = "معمولان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 892,
                            Name = "مومن آباد",
                            ProvinceId = 25,
                            Slug = "مومن-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 893,
                            Name = "نورآباد",
                            ProvinceId = 25,
                            Slug = "لرستان-نورآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 894,
                            Name = "ویسیان",
                            ProvinceId = 25,
                            Slug = "ویسیان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 895,
                            Name = "احمدسرگوراب",
                            ProvinceId = 26,
                            Slug = "احمدسرگوراب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 896,
                            Name = "اسالم",
                            ProvinceId = 26,
                            Slug = "اسالم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 897,
                            Name = "اطاقور",
                            ProvinceId = 26,
                            Slug = "اطاقور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 898,
                            Name = "املش",
                            ProvinceId = 26,
                            Slug = "املش",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 899,
                            Name = "آستارا",
                            ProvinceId = 26,
                            Slug = "آستارا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 900,
                            Name = "آستانه اشرفیه",
                            ProvinceId = 26,
                            Slug = "آستانه-اشرفیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 901,
                            Name = "بازار جمعه",
                            ProvinceId = 26,
                            Slug = "بازار-جمعه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 902,
                            Name = "بره سر",
                            ProvinceId = 26,
                            Slug = "بره-سر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 903,
                            Name = "بندرانزلی",
                            ProvinceId = 26,
                            Slug = "بندرانزلی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 906,
                            Name = "پره سر",
                            ProvinceId = 26,
                            Slug = "پره-سر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 907,
                            Name = "تالش",
                            ProvinceId = 26,
                            Slug = "تالش",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 908,
                            Name = "توتکابن",
                            ProvinceId = 26,
                            Slug = "توتکابن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 909,
                            Name = "جیرنده",
                            ProvinceId = 26,
                            Slug = "جیرنده",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 910,
                            Name = "چابکسر",
                            ProvinceId = 26,
                            Slug = "چابکسر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 911,
                            Name = "چاف و چمخاله",
                            ProvinceId = 26,
                            Slug = "چاف-و-چمخاله",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 912,
                            Name = "چوبر",
                            ProvinceId = 26,
                            Slug = "چوبر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 913,
                            Name = "حویق",
                            ProvinceId = 26,
                            Slug = "حویق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 914,
                            Name = "خشکبیجار",
                            ProvinceId = 26,
                            Slug = "خشکبیجار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 915,
                            Name = "خمام",
                            ProvinceId = 26,
                            Slug = "خمام",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 916,
                            Name = "دیلمان",
                            ProvinceId = 26,
                            Slug = "دیلمان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 917,
                            Name = "رانکوه",
                            ProvinceId = 26,
                            Slug = "رانکوه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 918,
                            Name = "رحیم آباد",
                            ProvinceId = 26,
                            Slug = "رحیم-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 919,
                            Name = "رستم آباد",
                            ProvinceId = 26,
                            Slug = "رستم-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 920,
                            Name = "رشت",
                            ProvinceId = 26,
                            Slug = "رشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 921,
                            Name = "رضوانشهر",
                            ProvinceId = 26,
                            Slug = "گیلان-رضوانشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 922,
                            Name = "رودبار",
                            ProvinceId = 26,
                            Slug = "گیلان-رودبار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 923,
                            Name = "رودبنه",
                            ProvinceId = 26,
                            Slug = "رودبنه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 924,
                            Name = "رودسر",
                            ProvinceId = 26,
                            Slug = "رودسر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 925,
                            Name = "سنگر",
                            ProvinceId = 26,
                            Slug = "سنگر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 926,
                            Name = "سیاهکل",
                            ProvinceId = 26,
                            Slug = "سیاهکل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 927,
                            Name = "شفت",
                            ProvinceId = 26,
                            Slug = "شفت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 928,
                            Name = "شلمان",
                            ProvinceId = 26,
                            Slug = "شلمان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 929,
                            Name = "صومعه سرا",
                            ProvinceId = 26,
                            Slug = "صومعه-سرا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 930,
                            Name = "فومن",
                            ProvinceId = 26,
                            Slug = "فومن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 931,
                            Name = "کلاچای",
                            ProvinceId = 26,
                            Slug = "کلاچای",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 932,
                            Name = "کوچصفهان",
                            ProvinceId = 26,
                            Slug = "کوچصفهان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 933,
                            Name = "کومله",
                            ProvinceId = 26,
                            Slug = "کومله",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 934,
                            Name = "کیاشهر",
                            ProvinceId = 26,
                            Slug = "کیاشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 935,
                            Name = "گوراب زرمیخ",
                            ProvinceId = 26,
                            Slug = "گوراب-زرمیخ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 936,
                            Name = "لاهیجان",
                            ProvinceId = 26,
                            Slug = "لاهیجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 937,
                            Name = "لشت نشا",
                            ProvinceId = 26,
                            Slug = "لشت-نشا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 938,
                            Name = "لنگرود",
                            ProvinceId = 26,
                            Slug = "لنگرود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 939,
                            Name = "لوشان",
                            ProvinceId = 26,
                            Slug = "لوشان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 940,
                            Name = "لولمان",
                            ProvinceId = 26,
                            Slug = "لولمان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 941,
                            Name = "لوندویل",
                            ProvinceId = 26,
                            Slug = "لوندویل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 942,
                            Name = "لیسار",
                            ProvinceId = 26,
                            Slug = "لیسار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 943,
                            Name = "ماسال",
                            ProvinceId = 26,
                            Slug = "ماسال",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 944,
                            Name = "ماسوله",
                            ProvinceId = 26,
                            Slug = "ماسوله",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 945,
                            Name = "مرجقل",
                            ProvinceId = 26,
                            Slug = "مرجقل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 946,
                            Name = "منجیل",
                            ProvinceId = 26,
                            Slug = "منجیل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 947,
                            Name = "واجارگاه",
                            ProvinceId = 26,
                            Slug = "واجارگاه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 948,
                            Name = "امیرکلا",
                            ProvinceId = 27,
                            Slug = "امیرکلا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 949,
                            Name = "ایزدشهر",
                            ProvinceId = 27,
                            Slug = "ایزدشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 950,
                            Name = "آلاشت",
                            ProvinceId = 27,
                            Slug = "آلاشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 951,
                            Name = "آمل",
                            ProvinceId = 27,
                            Slug = "آمل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 952,
                            Name = "بابل",
                            ProvinceId = 27,
                            Slug = "بابل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 953,
                            Name = "بابلسر",
                            ProvinceId = 27,
                            Slug = "بابلسر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 954,
                            Name = "بلده",
                            ProvinceId = 27,
                            Slug = "مازندران-بلده",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 955,
                            Name = "بهشهر",
                            ProvinceId = 27,
                            Slug = "بهشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 956,
                            Name = "بهنمیر",
                            ProvinceId = 27,
                            Slug = "بهنمیر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 957,
                            Name = "پل سفید",
                            ProvinceId = 27,
                            Slug = "پل-سفید",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 958,
                            Name = "تنکابن",
                            ProvinceId = 27,
                            Slug = "تنکابن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 959,
                            Name = "جویبار",
                            ProvinceId = 27,
                            Slug = "جویبار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 960,
                            Name = "چالوس",
                            ProvinceId = 27,
                            Slug = "چالوس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 961,
                            Name = "چمستان",
                            ProvinceId = 27,
                            Slug = "چمستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 962,
                            Name = "خرم آباد",
                            ProvinceId = 27,
                            Slug = "مازندران-خرم-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 963,
                            Name = "خلیل شهر",
                            ProvinceId = 27,
                            Slug = "خلیل-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 964,
                            Name = "خوش رودپی",
                            ProvinceId = 27,
                            Slug = "خوش-رودپی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 965,
                            Name = "دابودشت",
                            ProvinceId = 27,
                            Slug = "دابودشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 966,
                            Name = "رامسر",
                            ProvinceId = 27,
                            Slug = "رامسر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 967,
                            Name = "رستمکلا",
                            ProvinceId = 27,
                            Slug = "رستمکلا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 968,
                            Name = "رویان",
                            ProvinceId = 27,
                            Slug = "رویان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 969,
                            Name = "رینه",
                            ProvinceId = 27,
                            Slug = "رینه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 970,
                            Name = "زرگرمحله",
                            ProvinceId = 27,
                            Slug = "زرگرمحله",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 971,
                            Name = "زیرآب",
                            ProvinceId = 27,
                            Slug = "زیرآب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 972,
                            Name = "ساری",
                            ProvinceId = 27,
                            Slug = "ساری",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 973,
                            Name = "سرخرود",
                            ProvinceId = 27,
                            Slug = "سرخرود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 974,
                            Name = "سلمان شهر",
                            ProvinceId = 27,
                            Slug = "سلمان-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 975,
                            Name = "سورک",
                            ProvinceId = 27,
                            Slug = "سورک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 976,
                            Name = "شیرگاه",
                            ProvinceId = 27,
                            Slug = "شیرگاه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 977,
                            Name = "شیرود",
                            ProvinceId = 27,
                            Slug = "شیرود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 978,
                            Name = "عباس آباد",
                            ProvinceId = 27,
                            Slug = "عباس-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 979,
                            Name = "فریدونکنار",
                            ProvinceId = 27,
                            Slug = "فریدونکنار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 980,
                            Name = "فریم",
                            ProvinceId = 27,
                            Slug = "فریم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 981,
                            Name = "قائم شهر",
                            ProvinceId = 27,
                            Slug = "قائم-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 982,
                            Name = "کتالم",
                            ProvinceId = 27,
                            Slug = "کتالم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 983,
                            Name = "کلارآباد",
                            ProvinceId = 27,
                            Slug = "کلارآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 984,
                            Name = "کلاردشت",
                            ProvinceId = 27,
                            Slug = "کلاردشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 985,
                            Name = "کله بست",
                            ProvinceId = 27,
                            Slug = "کله-بست",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 986,
                            Name = "کوهی خیل",
                            ProvinceId = 27,
                            Slug = "کوهی-خیل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 987,
                            Name = "کیاسر",
                            ProvinceId = 27,
                            Slug = "کیاسر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 988,
                            Name = "کیاکلا",
                            ProvinceId = 27,
                            Slug = "کیاکلا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 989,
                            Name = "گتاب",
                            ProvinceId = 27,
                            Slug = "گتاب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 990,
                            Name = "گزنک",
                            ProvinceId = 27,
                            Slug = "گزنک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 991,
                            Name = "گلوگاه",
                            ProvinceId = 27,
                            Slug = "گلوگاه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 992,
                            Name = "محمودآباد",
                            ProvinceId = 27,
                            Slug = "مازندران-محمودآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 993,
                            Name = "مرزن آباد",
                            ProvinceId = 27,
                            Slug = "مرزن-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 994,
                            Name = "مرزیکلا",
                            ProvinceId = 27,
                            Slug = "مرزیکلا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 995,
                            Name = "نشتارود",
                            ProvinceId = 27,
                            Slug = "نشتارود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 996,
                            Name = "نکا",
                            ProvinceId = 27,
                            Slug = "نکا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 997,
                            Name = "نور",
                            ProvinceId = 27,
                            Slug = "نور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 998,
                            Name = "نوشهر",
                            ProvinceId = 27,
                            Slug = "نوشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 999,
                            Name = "اراک",
                            ProvinceId = 28,
                            Slug = "اراک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1000,
                            Name = "آستانه",
                            ProvinceId = 28,
                            Slug = "آستانه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1001,
                            Name = "آشتیان",
                            ProvinceId = 28,
                            Slug = "آشتیان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1002,
                            Name = "پرندک",
                            ProvinceId = 28,
                            Slug = "پرندک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1003,
                            Name = "تفرش",
                            ProvinceId = 28,
                            Slug = "تفرش",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1004,
                            Name = "توره",
                            ProvinceId = 28,
                            Slug = "توره",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1005,
                            Name = "جاورسیان",
                            ProvinceId = 28,
                            Slug = "جاورسیان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1006,
                            Name = "خشکرود",
                            ProvinceId = 28,
                            Slug = "خشکرود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1007,
                            Name = "خمین",
                            ProvinceId = 28,
                            Slug = "خمین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1008,
                            Name = "خنداب",
                            ProvinceId = 28,
                            Slug = "خنداب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1009,
                            Name = "داودآباد",
                            ProvinceId = 28,
                            Slug = "داودآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1010,
                            Name = "دلیجان",
                            ProvinceId = 28,
                            Slug = "دلیجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1011,
                            Name = "رازقان",
                            ProvinceId = 28,
                            Slug = "رازقان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1012,
                            Name = "زاویه",
                            ProvinceId = 28,
                            Slug = "زاویه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1013,
                            Name = "ساروق",
                            ProvinceId = 28,
                            Slug = "ساروق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1014,
                            Name = "ساوه",
                            ProvinceId = 28,
                            Slug = "ساوه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1015,
                            Name = "سنجان",
                            ProvinceId = 28,
                            Slug = "سنجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1016,
                            Name = "شازند",
                            ProvinceId = 28,
                            Slug = "شازند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1017,
                            Name = "غرق آباد",
                            ProvinceId = 28,
                            Slug = "غرق-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1018,
                            Name = "فرمهین",
                            ProvinceId = 28,
                            Slug = "فرمهین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1019,
                            Name = "قورچی باشی",
                            ProvinceId = 28,
                            Slug = "قورچی-باشی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1020,
                            Name = "کرهرود",
                            ProvinceId = 28,
                            Slug = "کرهرود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1021,
                            Name = "کمیجان",
                            ProvinceId = 28,
                            Slug = "کمیجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1022,
                            Name = "مامونیه",
                            ProvinceId = 28,
                            Slug = "مامونیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1023,
                            Name = "محلات",
                            ProvinceId = 28,
                            Slug = "محلات",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1024,
                            Name = "مهاجران",
                            ProvinceId = 28,
                            Slug = "مهاجران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1025,
                            Name = "میلاجرد",
                            ProvinceId = 28,
                            Slug = "میلاجرد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1026,
                            Name = "نراق",
                            ProvinceId = 28,
                            Slug = "نراق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1027,
                            Name = "نوبران",
                            ProvinceId = 28,
                            Slug = "نوبران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1028,
                            Name = "نیمور",
                            ProvinceId = 28,
                            Slug = "نیمور",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1029,
                            Name = "هندودر",
                            ProvinceId = 28,
                            Slug = "هندودر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1030,
                            Name = "ابوموسی",
                            ProvinceId = 29,
                            Slug = "ابوموسی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1031,
                            Name = "بستک",
                            ProvinceId = 29,
                            Slug = "بستک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1032,
                            Name = "بندرجاسک",
                            ProvinceId = 29,
                            Slug = "بندرجاسک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1033,
                            Name = "بندرچارک",
                            ProvinceId = 29,
                            Slug = "بندرچارک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1034,
                            Name = "بندرخمیر",
                            ProvinceId = 29,
                            Slug = "بندرخمیر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1035,
                            Name = "بندرعباس",
                            ProvinceId = 29,
                            Slug = "بندرعباس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1036,
                            Name = "بندرلنگه",
                            ProvinceId = 29,
                            Slug = "بندرلنگه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1037,
                            Name = "بیکا",
                            ProvinceId = 29,
                            Slug = "بیکا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1038,
                            Name = "پارسیان",
                            ProvinceId = 29,
                            Slug = "پارسیان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1039,
                            Name = "تخت",
                            ProvinceId = 29,
                            Slug = "تخت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1040,
                            Name = "جناح",
                            ProvinceId = 29,
                            Slug = "جناح",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1041,
                            Name = "حاجی آباد",
                            ProvinceId = 29,
                            Slug = "هرمزگان-حاجی-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1042,
                            Name = "درگهان",
                            ProvinceId = 29,
                            Slug = "درگهان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1043,
                            Name = "دهبارز",
                            ProvinceId = 29,
                            Slug = "دهبارز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1044,
                            Name = "رویدر",
                            ProvinceId = 29,
                            Slug = "رویدر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1045,
                            Name = "زیارتعلی",
                            ProvinceId = 29,
                            Slug = "زیارتعلی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1046,
                            Name = "سردشت",
                            ProvinceId = 29,
                            Slug = "هرمزگان-سردشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1047,
                            Name = "سندرک",
                            ProvinceId = 29,
                            Slug = "سندرک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1048,
                            Name = "سوزا",
                            ProvinceId = 29,
                            Slug = "سوزا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1049,
                            Name = "سیریک",
                            ProvinceId = 29,
                            Slug = "سیریک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1050,
                            Name = "فارغان",
                            ProvinceId = 29,
                            Slug = "فارغان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1051,
                            Name = "فین",
                            ProvinceId = 29,
                            Slug = "فین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1052,
                            Name = "قشم",
                            ProvinceId = 29,
                            Slug = "قشم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1053,
                            Name = "قلعه قاضی",
                            ProvinceId = 29,
                            Slug = "قلعه-قاضی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1054,
                            Name = "کنگ",
                            ProvinceId = 29,
                            Slug = "کنگ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1055,
                            Name = "کوشکنار",
                            ProvinceId = 29,
                            Slug = "کوشکنار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1056,
                            Name = "کیش",
                            ProvinceId = 29,
                            Slug = "کیش",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1057,
                            Name = "گوهران",
                            ProvinceId = 29,
                            Slug = "گوهران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1058,
                            Name = "میناب",
                            ProvinceId = 29,
                            Slug = "میناب",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1059,
                            Name = "هرمز",
                            ProvinceId = 29,
                            Slug = "هرمز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1060,
                            Name = "هشتبندی",
                            ProvinceId = 29,
                            Slug = "هشتبندی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1061,
                            Name = "ازندریان",
                            ProvinceId = 30,
                            Slug = "ازندریان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1062,
                            Name = "اسدآباد",
                            ProvinceId = 30,
                            Slug = "اسدآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1063,
                            Name = "برزول",
                            ProvinceId = 30,
                            Slug = "برزول",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1064,
                            Name = "بهار",
                            ProvinceId = 30,
                            Slug = "بهار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1065,
                            Name = "تویسرکان",
                            ProvinceId = 30,
                            Slug = "تویسرکان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1066,
                            Name = "جورقان",
                            ProvinceId = 30,
                            Slug = "جورقان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1067,
                            Name = "جوکار",
                            ProvinceId = 30,
                            Slug = "جوکار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1068,
                            Name = "دمق",
                            ProvinceId = 30,
                            Slug = "دمق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1069,
                            Name = "رزن",
                            ProvinceId = 30,
                            Slug = "رزن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1070,
                            Name = "زنگنه",
                            ProvinceId = 30,
                            Slug = "زنگنه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1071,
                            Name = "سامن",
                            ProvinceId = 30,
                            Slug = "سامن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1072,
                            Name = "سرکان",
                            ProvinceId = 30,
                            Slug = "سرکان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1073,
                            Name = "شیرین سو",
                            ProvinceId = 30,
                            Slug = "شیرین-سو",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1074,
                            Name = "صالح آباد",
                            ProvinceId = 30,
                            Slug = "همدان-صالح-آباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1075,
                            Name = "فامنین",
                            ProvinceId = 30,
                            Slug = "فامنین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1076,
                            Name = "فرسفج",
                            ProvinceId = 30,
                            Slug = "فرسفج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1077,
                            Name = "فیروزان",
                            ProvinceId = 30,
                            Slug = "فیروزان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1078,
                            Name = "قروه درجزین",
                            ProvinceId = 30,
                            Slug = "قروه-درجزین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1079,
                            Name = "قهاوند",
                            ProvinceId = 30,
                            Slug = "قهاوند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1080,
                            Name = "کبودر آهنگ",
                            ProvinceId = 30,
                            Slug = "کبودر-آهنگ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1081,
                            Name = "گل تپه",
                            ProvinceId = 30,
                            Slug = "گل-تپه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1082,
                            Name = "گیان",
                            ProvinceId = 30,
                            Slug = "گیان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1083,
                            Name = "لالجین",
                            ProvinceId = 30,
                            Slug = "لالجین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1084,
                            Name = "مریانج",
                            ProvinceId = 30,
                            Slug = "مریانج",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1085,
                            Name = "ملایر",
                            ProvinceId = 30,
                            Slug = "ملایر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1086,
                            Name = "نهاوند",
                            ProvinceId = 30,
                            Slug = "نهاوند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1087,
                            Name = "همدان",
                            ProvinceId = 30,
                            Slug = "همدان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1088,
                            Name = "ابرکوه",
                            ProvinceId = 31,
                            Slug = "ابرکوه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1089,
                            Name = "احمدآباد",
                            ProvinceId = 31,
                            Slug = "احمدآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1090,
                            Name = "اردکان",
                            ProvinceId = 31,
                            Slug = "یزد-اردکان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1091,
                            Name = "اشکذر",
                            ProvinceId = 31,
                            Slug = "اشکذر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1092,
                            Name = "بافق",
                            ProvinceId = 31,
                            Slug = "بافق",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1093,
                            Name = "بفروئیه",
                            ProvinceId = 31,
                            Slug = "بفروئیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1094,
                            Name = "بهاباد",
                            ProvinceId = 31,
                            Slug = "بهاباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1095,
                            Name = "تفت",
                            ProvinceId = 31,
                            Slug = "تفت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1096,
                            Name = "حمیدیا",
                            ProvinceId = 31,
                            Slug = "حمیدیا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1097,
                            Name = "خضرآباد",
                            ProvinceId = 31,
                            Slug = "خضرآباد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1098,
                            Name = "دیهوک",
                            ProvinceId = 31,
                            Slug = "دیهوک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1099,
                            Name = "زارچ",
                            ProvinceId = 31,
                            Slug = "زارچ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1100,
                            Name = "شاهدیه",
                            ProvinceId = 31,
                            Slug = "شاهدیه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1101,
                            Name = "طبس",
                            ProvinceId = 31,
                            Slug = "یزد-طبس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1103,
                            Name = "عقدا",
                            ProvinceId = 31,
                            Slug = "عقدا",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1104,
                            Name = "مروست",
                            ProvinceId = 31,
                            Slug = "مروست",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1105,
                            Name = "مهردشت",
                            ProvinceId = 31,
                            Slug = "مهردشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1106,
                            Name = "مهریز",
                            ProvinceId = 31,
                            Slug = "مهریز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1107,
                            Name = "میبد",
                            ProvinceId = 31,
                            Slug = "میبد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1108,
                            Name = "ندوشن",
                            ProvinceId = 31,
                            Slug = "ندوشن",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1109,
                            Name = "نیر",
                            ProvinceId = 31,
                            Slug = "یزد-نیر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1110,
                            Name = "هرات",
                            ProvinceId = 31,
                            Slug = "هرات",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1111,
                            Name = "یزد",
                            ProvinceId = 31,
                            Slug = "یزد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1116,
                            Name = "پرند",
                            ProvinceId = 8,
                            Slug = "پرند",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1117,
                            Name = "فردیس",
                            ProvinceId = 5,
                            Slug = "فردیس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1118,
                            Name = "مارلیک",
                            ProvinceId = 5,
                            Slug = "مارلیک",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1119,
                            Name = "سادات شهر",
                            ProvinceId = 27,
                            Slug = "سادات-شهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1121,
                            Name = "زیباکنار",
                            ProvinceId = 26,
                            Slug = "زیباکنار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1135,
                            Name = "کردان",
                            ProvinceId = 5,
                            Slug = "کردان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1137,
                            Name = "ساوجبلاغ",
                            ProvinceId = 5,
                            Slug = "ساوجبلاغ",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1138,
                            Name = "تهران دشت",
                            ProvinceId = 5,
                            Slug = "تهران-دشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1150,
                            Name = "گلبهار",
                            ProvinceId = 11,
                            Slug = "گلبهار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1153,
                            Name = "قیامدشت",
                            ProvinceId = 8,
                            Slug = "قیامدشت",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1155,
                            Name = "بینالود",
                            ProvinceId = 11,
                            Slug = "بینالود",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1159,
                            Name = "پیربازار",
                            ProvinceId = 26,
                            Slug = "پیربازار",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 1160,
                            Name = "رضوانشهر",
                            ProvinceId = 31,
                            Slug = "رضوانشهر",
                            Status = "Active"
                        });
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Coupon", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Discriminator")
                        .IsRequired()
                        .HasMaxLength(21)
                        .HasColumnType("nvarchar(21)");

                    b.Property<DateTime>("ExpiryDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Quantity")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("Coupon");

                    b.HasDiscriminator<string>("Discriminator").HasValue("Coupon");

                    b.UseTphMappingStrategy();
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Media", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Alt")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Extension")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FullPath")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RelativePath")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("Size")
                        .HasColumnType("bigint");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("Medias");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.OTP", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsUsed")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("OTPs");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Order", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("AddressId")
                        .HasColumnType("int");

                    b.Property<int>("Courier")
                        .HasColumnType("int");

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Ip")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastStatusUpdatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("OrderStatus")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PackageSize")
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("TotalDiscountPrice")
                        .HasColumnType("bigint");

                    b.Property<long>("TotalFinalPrice")
                        .HasColumnType("bigint");

                    b.Property<long>("TotalProductPrice")
                        .HasColumnType("bigint");

                    b.Property<long>("TotalSendPrice")
                        .HasColumnType("bigint");

                    b.Property<long>("TotalTaxPrice")
                        .HasColumnType("bigint");

                    b.Property<string>("TrackingCode")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("Weight")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("AddressId");

                    b.HasIndex("UserId");

                    b.ToTable("Orders");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.OrderItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("Count")
                        .HasColumnType("int");

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("DiscountPrice")
                        .HasColumnType("bigint");

                    b.Property<long>("FinalPrice")
                        .HasColumnType("bigint");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("OrderId")
                        .HasColumnType("int");

                    b.Property<long>("PriceWithCount")
                        .HasColumnType("bigint");

                    b.Property<int?>("ProductId")
                        .HasColumnType("int");

                    b.Property<int?>("ProductVariantId")
                        .HasColumnType("int");

                    b.Property<long>("RawPrice")
                        .HasColumnType("bigint");

                    b.Property<long>("RawPriceVariant")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("OrderId");

                    b.HasIndex("ProductId");

                    b.HasIndex("ProductVariantId");

                    b.ToTable("OrderItems");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Product", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("Price")
                        .HasColumnType("bigint");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Stock")
                        .HasColumnType("int");

                    b.Property<int>("TotalSales")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("Products");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.ProductAttribute", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ProductId")
                        .HasColumnType("int");

                    b.Property<string>("Value")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("ProductId");

                    b.ToTable("ProductAttributes");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.ProductReview", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ProductId")
                        .HasColumnType("int");

                    b.Property<int>("Rating")
                        .HasColumnType("int");

                    b.Property<string>("ReviewText")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("ProductId");

                    b.HasIndex("UserId");

                    b.ToTable("ProductReviews");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.ProductVariant", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("PriceAdjustment")
                        .HasColumnType("bigint");

                    b.Property<int>("ProductId")
                        .HasColumnType("int");

                    b.Property<int>("Stock")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ProductId");

                    b.ToTable("ProductVariants");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Province", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Slug")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Provinces");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "آذربایجان شرقی",
                            Slug = "آذربایجان-شرقی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 2,
                            Name = "آذربایجان غربی",
                            Slug = "آذربایجان-غربی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 3,
                            Name = "اردبیل",
                            Slug = "اردبیل",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 4,
                            Name = "اصفهان",
                            Slug = "اصفهان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 5,
                            Name = "البرز",
                            Slug = "البرز",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 6,
                            Name = "ایلام",
                            Slug = "ایلام",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 7,
                            Name = "بوشهر",
                            Slug = "بوشهر",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 8,
                            Name = "تهران",
                            Slug = "تهران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 9,
                            Name = "چهارمحال و بختیاری",
                            Slug = "چهارمحال-بختیاری",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 10,
                            Name = "خراسان جنوبی",
                            Slug = "خراسان-جنوبی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 11,
                            Name = "خراسان رضوی",
                            Slug = "خراسان-رضوی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 12,
                            Name = "خراسان شمالی",
                            Slug = "خراسان-شمالی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 13,
                            Name = "خوزستان",
                            Slug = "خوزستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 14,
                            Name = "زنجان",
                            Slug = "زنجان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 15,
                            Name = "سمنان",
                            Slug = "سمنان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 16,
                            Name = "سیستان و بلوچستان",
                            Slug = "سیستان-بلوچستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 17,
                            Name = "فارس",
                            Slug = "فارس",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 18,
                            Name = "قزوین",
                            Slug = "قزوین",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 19,
                            Name = "قم",
                            Slug = "قم",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 20,
                            Name = "کردستان",
                            Slug = "کردستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 21,
                            Name = "کرمان",
                            Slug = "کرمان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 22,
                            Name = "کرمانشاه",
                            Slug = "کرمانشاه",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 23,
                            Name = "کهگیلویه و بویراحمد",
                            Slug = "کهگیلویه-بویراحمد",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 24,
                            Name = "گلستان",
                            Slug = "گلستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 25,
                            Name = "لرستان",
                            Slug = "لرستان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 26,
                            Name = "گیلان",
                            Slug = "گیلان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 27,
                            Name = "مازندران",
                            Slug = "مازندران",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 28,
                            Name = "مرکزی",
                            Slug = "مرکزی",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 29,
                            Name = "هرمزگان",
                            Slug = "هرمزگان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 30,
                            Name = "همدان",
                            Slug = "همدان",
                            Status = "Active"
                        },
                        new
                        {
                            Id = 31,
                            Name = "یزد",
                            Slug = "یزد",
                            Status = "Active"
                        });
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Return", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("OrderId")
                        .HasColumnType("int");

                    b.Property<int>("OrderItemId")
                        .HasColumnType("int");

                    b.Property<int>("OrderStatus")
                        .HasColumnType("int");

                    b.Property<int>("ProductId")
                        .HasColumnType("int");

                    b.Property<string>("ReturnReason")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("OrderId");

                    b.HasIndex("OrderItemId");

                    b.HasIndex("ProductId");

                    b.HasIndex("UserId");

                    b.ToTable("Returns");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Subscription", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Subscriptions");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.TodoItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("Done")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ListId")
                        .HasColumnType("int");

                    b.Property<string>("Note")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Priority")
                        .HasColumnType("int");

                    b.Property<DateTime?>("Reminder")
                        .HasColumnType("datetime2");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.HasKey("Id");

                    b.HasIndex("ListId");

                    b.ToTable("TodoItems");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.TodoList", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.HasKey("Id");

                    b.ToTable("TodoLists");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.User", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Discriminator")
                        .IsRequired()
                        .HasMaxLength(8)
                        .HasColumnType("nvarchar(8)");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("FirstName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("IsActive")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsAdmin")
                        .HasColumnType("bit");

                    b.Property<string>("LastName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<long>("NationalCode")
                        .HasColumnType("bigint");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("Users", (string)null);

                    b.HasDiscriminator<string>("Discriminator").HasValue("User");

                    b.UseTphMappingStrategy();
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Wishlist", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("Created")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("LastModified")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ProductId")
                        .HasColumnType("int");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("ProductId");

                    b.HasIndex("UserId");

                    b.ToTable("Wishlists");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.CouponFixedAmount", b =>
                {
                    b.HasBaseType("NiceShop.Domain.Entities.Coupon");

                    b.Property<long>("FixedAmount")
                        .HasColumnType("bigint");

                    b.HasDiscriminator().HasValue("CouponFixedAmount");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.CouponPercentage", b =>
                {
                    b.HasBaseType("NiceShop.Domain.Entities.Coupon");

                    b.Property<int>("Percentage")
                        .HasColumnType("int");

                    b.HasDiscriminator().HasValue("CouponPercentage");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Customer", b =>
                {
                    b.HasBaseType("NiceShop.Domain.Entities.User");

                    b.HasDiscriminator().HasValue("Customer");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Employee", b =>
                {
                    b.HasBaseType("NiceShop.Domain.Entities.User");

                    b.HasDiscriminator().HasValue("Employee");
                });

            modelBuilder.Entity("ArticleCategory", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Article", null)
                        .WithMany()
                        .HasForeignKey("ArticlesId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.Category", null)
                        .WithMany()
                        .HasForeignKey("CategoriesId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CategoryProduct", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Category", null)
                        .WithMany()
                        .HasForeignKey("CategoriesId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.Product", null)
                        .WithMany()
                        .HasForeignKey("ProductsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CouponProduct", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Coupon", null)
                        .WithMany()
                        .HasForeignKey("CouponsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.Product", null)
                        .WithMany()
                        .HasForeignKey("ProductsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("MediaProduct", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Media", null)
                        .WithMany()
                        .HasForeignKey("MediasId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.Product", null)
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Address", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.City", "City")
                        .WithMany()
                        .HasForeignKey("CityId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.Province", "Province")
                        .WithMany()
                        .HasForeignKey("ProvinceId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.User", "User")
                        .WithMany("Addresses")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("City");

                    b.Navigation("Province");

                    b.Navigation("User");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Article", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Media", "Media")
                        .WithMany()
                        .HasForeignKey("MediaId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("NiceShop.Domain.Entities.User", "User")
                        .WithMany("Articles")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Media");

                    b.Navigation("User");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Basket", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Coupon", "Coupon")
                        .WithMany()
                        .HasForeignKey("CouponId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("NiceShop.Domain.Entities.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Coupon");

                    b.Navigation("User");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.BasketItem", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Basket", "Basket")
                        .WithMany("BasketItems")
                        .HasForeignKey("BasketId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.Product", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.ProductVariant", "ProductVariant")
                        .WithMany()
                        .HasForeignKey("ProductVariantId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Basket");

                    b.Navigation("Product");

                    b.Navigation("ProductVariant");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Category", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Category", null)
                        .WithMany("SubCategories")
                        .HasForeignKey("ParentCategoryId")
                        .OnDelete(DeleteBehavior.NoAction);
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.City", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Province", "Province")
                        .WithMany("Cities")
                        .HasForeignKey("ProvinceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Province");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Media", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.User", "User")
                        .WithMany("Medias")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.OTP", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.User", "User")
                        .WithOne()
                        .HasForeignKey("NiceShop.Domain.Entities.OTP", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Order", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Address", "Address")
                        .WithMany()
                        .HasForeignKey("AddressId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("NiceShop.Domain.Entities.User", "User")
                        .WithMany("Orders")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Address");

                    b.Navigation("User");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.OrderItem", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Order", "Order")
                        .WithMany("OrderItems")
                        .HasForeignKey("OrderId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.Product", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("NiceShop.Domain.Entities.ProductVariant", "ProductVariant")
                        .WithMany()
                        .HasForeignKey("ProductVariantId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Order");

                    b.Navigation("Product");

                    b.Navigation("ProductVariant");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.ProductAttribute", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Product", null)
                        .WithMany("ProductAttributes")
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.ProductReview", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Product", null)
                        .WithMany("ProductReviews")
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.User", "User")
                        .WithMany("ProductReviews")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.ProductVariant", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Product", null)
                        .WithMany("ProductVariants")
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Return", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Order", "Order")
                        .WithMany()
                        .HasForeignKey("OrderId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.OrderItem", "OrderItem")
                        .WithMany()
                        .HasForeignKey("OrderItemId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.Product", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.User", "User")
                        .WithMany("Returns")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Order");

                    b.Navigation("OrderItem");

                    b.Navigation("Product");

                    b.Navigation("User");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.TodoItem", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.TodoList", "List")
                        .WithMany("Items")
                        .HasForeignKey("ListId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("List");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.TodoList", b =>
                {
                    b.OwnsOne("NiceShop.Domain.ValueObjects.Colour", "Colour", b1 =>
                        {
                            b1.Property<int>("TodoListId")
                                .HasColumnType("int");

                            b1.Property<string>("Code")
                                .IsRequired()
                                .HasColumnType("nvarchar(max)");

                            b1.HasKey("TodoListId");

                            b1.ToTable("TodoLists");

                            b1.WithOwner()
                                .HasForeignKey("TodoListId");
                        });

                    b.Navigation("Colour")
                        .IsRequired();
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.User", b =>
                {
                    b.OwnsOne("NiceShop.Domain.Entities.Social", "Social", b1 =>
                        {
                            b1.Property<string>("UserId")
                                .HasColumnType("nvarchar(450)");

                            b1.Property<string>("Instagram")
                                .HasColumnType("nvarchar(max)");

                            b1.Property<string>("Telegram")
                                .HasColumnType("nvarchar(max)");

                            b1.Property<string>("Twitter")
                                .HasColumnType("nvarchar(max)");

                            b1.HasKey("UserId");

                            b1.ToTable("Users");

                            b1.WithOwner()
                                .HasForeignKey("UserId");
                        });

                    b.Navigation("Social")
                        .IsRequired();
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Wishlist", b =>
                {
                    b.HasOne("NiceShop.Domain.Entities.Product", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("NiceShop.Domain.Entities.User", null)
                        .WithMany("Wishlists")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Product");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Basket", b =>
                {
                    b.Navigation("BasketItems");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Category", b =>
                {
                    b.Navigation("SubCategories");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Order", b =>
                {
                    b.Navigation("OrderItems");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Product", b =>
                {
                    b.Navigation("ProductAttributes");

                    b.Navigation("ProductReviews");

                    b.Navigation("ProductVariants");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.Province", b =>
                {
                    b.Navigation("Cities");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.TodoList", b =>
                {
                    b.Navigation("Items");
                });

            modelBuilder.Entity("NiceShop.Domain.Entities.User", b =>
                {
                    b.Navigation("Addresses");

                    b.Navigation("Articles");

                    b.Navigation("Medias");

                    b.Navigation("Orders");

                    b.Navigation("ProductReviews");

                    b.Navigation("Returns");

                    b.Navigation("Wishlists");
                });
#pragma warning restore 612, 618
        }
    }
}
